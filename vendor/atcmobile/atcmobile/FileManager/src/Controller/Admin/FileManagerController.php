...
namespace Atcmobapp\FileManager\Controller\Admin;
...
use Atcmobapp\FileManager\Utility\FileManager;

/**
 * FileManager Controller
 *
 * @category FileManager.Controller
 * @package  Atcmobapp.FileManager.Controller
 * @version  1.0
...
class FileManagerController extends AppController
{
    ...
    public function initialize()
    {
        ...
        $this->viewBuilder()
            ->setHelpers([
                'Atcmobapp/Core.Image',
                'Atcmobapp/FileManager.FileManager',
            ]);
    }
    ...
    public function browse()
    {
        ...
        if (preg_match($regex, $path) == false) {
            $this->Flash->error(__d('atcmobile', 'Path %s is restricted', $path));
            $path = APP;
        }
        ...
        if (in_array(basename($path), $blacklist) || preg_match($regex, $path)) {
            $this->Flash->error(__d('atcmobile', sprintf('Path %s is restricted', $path)));
            $path = dirname($path);
        }
    ...
    public function editFile()
    {
        ...
        if (!empty($this->getRequest()->getData())) {
            if ($this->file->write($this->getRequest()->getData('content'))) {
                $this->Flash->success(__d('atcmobile', 'File saved successfully'));
            }
        }
    ...
    {
        $this->set('title_for_layout', __d('atcmobile', 'Upload'));
        ...
        if (isset($postFile['tmp_name']) &&
            ...
            $this->Flash->success(__d('atcmobile', 'File uploaded successfully.'));
    ...
    public function deleteFile()
    {
        ...
        if (file_exists($path) && unlink($path)) {
            $this->Flash->success(__d('atcmobile', 'File deleted'));
        } else {
            $this->Flash->error(__d('atcmobile', 'An error occured'));
        }
    ...
    public function deleteDirectory()
    {
        ...
        if (is_dir($path) && $folder->delete($path)) {
            $this->Flash->success(__d('atcmobile', 'Directory deleted'));
        } else {
            $this->Flash->error(__d('atcmobile', 'An error occured'));
        }
    ...
    public function rename()
    {
        ...
        if (!$this->FileManager->isEditable($path)) {
            $this->Flash->error(__d('atcmobile', 'Path "%s" cannot be renamed', $path));
        ...
        if ($this->getRequest()->is('post') || $this->getRequest()->is('put')) {
            if (!is_null($this->getRequest()->data('name')) &&
                ...
                if ($oldName !== $newName) {
                    if ($fileExists) {
                        $message = __d('atcmobile', '%s already exists', $newName);
                        $alertType = 'error';
                    } else {
                        if ($this->FileManager->rename($path, $newPath)) {
                            $message = __d('atcmobile', '"%s" has been renamed to "%s"', $oldName, $newName);
                            $alertType = 'success';
                        } else {
                            $message = __d('atcmobile', 'Could not rename "%s" to "%s"', $oldName, $newName);
                            $alertType = 'error';
                        }
                    }
                } else {
                    $message = __d('atcmobile', 'Name has not changed');
                    $alertType = 'warning';
                }
    ...
    public function createDirectory()
    {
        ...
        if (!empty($this->getRequest()->data)) {
            ...
            if ($this->folder->create($path . $this->getRequest()->data['name'])) {
                $this->Flash->success(__d('atcmobile', 'Directory created successfully.'));
                $redirectUrl = $this->_browsePathUrl($path);
    ...
    public function createFile()
    {
        ...
        if (!empty($this->getRequest()->data)) {
            if (file_put_contents($path . $this->getRequest()->data['name'], $this->getRequest()->data['content'])) {
                $this->Flash->success(__d('atcmobile', 'File created successfully.'));
                $redirectUrl = $this->_browsePathUrl($path);
