...
<div class="input text clearfix fileupload-buttonbar">
    <div class="col-8">
        ...
        <span class="btn btn-outline-success fileinput-button">
            <?php echo $addIcon; ?>
            <span><?php echo __d('atcmobile', 'Add files'); ?></span>
        ...
        <td>
            {% if (!i && !o.options.autoUpload) { %}
                <button class="btn btn-sm btn-primary start" disabled
                    title="<?php echo __d('atcmobile', 'Start'); ?>"
                >
                    <?php echo $uploadIcon; ?>
                </button>
            {% } %}
            {% if (!i) { %}
                <button class="btn btn-sm btn-warning cancel"
                    title="<?php echo __d('atcmobile', 'Cancel'); ?>"
                >
                    <?php echo $cancelIcon; ?>
                </button>
            {% } %}
        ...
        <td>
            {% if (file.deleteUrl) { %}
                <button class="btn btn-danger delete" data-type="{%=file.deleteType%}" data-url="{%=file.deleteUrl%}"{% if (file.deleteWithCredentials) { %} data-xhr-fields='{"withCredentials":true}'{% } %}
                    title="<?php echo __d('atcmobile', 'Delete'); ?>"
                >
                    <?= $cancelIcon ?>
                </button>
