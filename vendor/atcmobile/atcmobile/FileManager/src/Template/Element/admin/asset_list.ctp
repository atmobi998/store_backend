...
$this->Atcmobapp->adminScript([
    'Atcmobapp/FileManager.admin.js',
    'Atcmobapp/FileManager.assets.js',
]);
...
if (!isset($attachments)) :
    $Attachment = TableRegistry::get('Atcmobapp/FileManager.Attachments');
    if (isset($id)) :
...
//if (!$this->helpers()->loaded('AssetsImage')) {
    $this->loadHelper('Atcmobapp/FileManager.AssetsImage');
//}
...
foreach ($attachments as $attachment) :
    ...
    elseif ($mimeType === 'video') :
        ...
        $imgUrl = $this->Html->image('Atcmobapp/Core./img/icons/page_white.png') . ' ';
    ...
    $action[] = $this->Atcmobapp->adminRowAction('', $attachmentEditUrl, [
        'icon' => 'update',
        'escapeTitle' => false,
        'class' => 'asset-edit',
        'data-id' => $attachment->id,
        'tooltip' => __d('atcmobile', 'Edit Attachment'),
    ]);

    if ($mimeType === 'image'):
        $action[] = $this->Atcmobapp->adminRowAction('', $detailUrl, [
            'icon' => 'suitcase',
            'escapeTitle' => false,
            'data-toggle' => 'browse',
    ...
    if ($mimeType === 'image' || $mimeType == 'video'):
        $action[] = $this->Atcmobapp->adminRowAction('', $changeTypeUrl, [
            'icon' => 'star',
            'escapeTitle' => false,
            'class' => 'set-featured-image',
            'data-pk' => $attachment->asset_usage->id,
            'tooltip' => __d('atcmobile', 'Set as FeaturedImage'),
        ]);
    endif;

    $action[] = $this->Atcmobapp->adminRowAction('', $unregisterUsageUrl, [
        'icon' => 'minus',
        'escapeTitle' => false,
        'class' => 'unregister-usage red',
        'data-id' => $attachment->asset_usage->id,
        'tooltip' => __d('atcmobile', 'Unregister asset from this resource'),
    ]);
...
if (!isset($_assetButtons)) :
    ...
    echo $this->Atcmobapp->adminAction(
        ...
        [
            ...
            'iconSize' => 'small',
            'data-toggle' => 'refresh',
            'tooltip' => __d('atcmobile', 'Reload asset list for this content'),
        ]
    );
    echo $this->Atcmobapp->adminAction(
        __d('atcmobile', 'Browse'),
        $browseUrl,
        [
            'div' => false,
            'icon' => 'folder-open',
            'iconSize' => 'small',
            'data-toggle' => 'browse',
            'tooltip' => __d('atcmobile', 'Browse available assets'),
        ]
    );
    echo $this->Atcmobapp->adminAction(
        ...
        [
            ...
            'iconSize' => 'small',
            'data-toggle' => 'browse',
            'tooltip' => __d('atcmobile', 'Upload new asset for this content'),
        ]
    );
    $this->end();
