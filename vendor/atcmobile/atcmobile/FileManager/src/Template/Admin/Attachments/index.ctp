...
$this->Atcmobapp->adminScript('Atcmobapp/FileManager.admin');
$this->Atcmobapp->adminScript('Atcmobapp/FileManager.assets');

$this->extend('Atcmobapp/Core./Common/admin_index');

$this->Breadcrumbs
    ->add(__d('atcmobile', 'Attachments'), $this->getRequest()->getUri()->getPath());
...
echo $this->Atcmobapp->adminAction(
    __d('atcmobile', 'New ' . __d('atcmobile', 'Attachment')),
...
$this->append('table-heading');
    ...
    $this->append('search', $this->element('Atcmobapp/Core.admin/search'));
    ...
    foreach ($attachments as $attachment) {
        ...
        if ($mimeType == 'image' || $mimeType == 'video') {
            ...
            $actions[] = $this->Atcmobapp->adminRowActions($attachment->id);
        ...
        $actions[] = $this->Atcmobapp->adminRowAction('', $editUrl, [
            'icon' => 'update',
            'escapeTitle' => false,
        ...
        $actions[] = $this->Atcmobapp->adminRowAction('', $deleteUrl, [
            'icon' => 'delete',
            'escapeTitle' => false,
        ...
        switch ($mimeType) {
            ...
            default:
                $thumbnail = sprintf(
                    '%s %s (%s)',
                    $this->Html->image('Atcmobapp/Core./img/icons/page_white.png', [
                        'alt' => $mimeType,
                    ]),
                    $mimeType,
