...
namespace Atcmobapp\FileManager\View\Helper;
 ...
 *
 * @category Helper
 * @package  Atcmobapp.FileManager.View.Helper
 * @version  1.0
...
class FileManagerHelper extends Helper
{
     ...
     *
     * @var \Atcmobapp\FileManager\Utility\FileManager
     */
    ...
    public function link($title, $url, $path, $pathKey = 'path')
    {
        ...
        if (isset($url['action']) && in_array($url['action'], $this->__postLinkActions)) {
            $output = $this->Form->postLink($title, $url, ['data' => compact('path'), 'escape' => true], __d('atcmobile', 'Are you sure?'));
        } else {
            $url[$pathKey] = $path;
    ...
    /**
     * @see \Atcmobapp\FileManager\Utility\FileManager::isEditable()
     */
    ...
    /**
     * @see \Atcmobapp\FileManager\Utility\FileManager::isDeletable()
     */
