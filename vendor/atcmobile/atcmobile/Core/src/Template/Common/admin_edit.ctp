...
if (isset(${Inflector::variable(Inflector::singularize($this->name))})) :
    ...
    if ($entity instanceof EntityInterface) :
        $what = !$entity->isNew() ? __d('atcmobile', 'Edit') : __d('atcmobile', 'Add');
    else :
        $what = __d('atcmobile', 'Edit');
    endif;
endif;
...
if (!$this->exists('left-column')) :
    ...
    if (empty($tabHeading)) :
        $tabHeading = $this->Atcmobapp->adminTab(__d('atcmobile', $modelClass), "#$tabId");
    endif;
    $tabHeading .= $this->Atcmobapp->adminTabs();
...
if (!$this->exists('right-column')) :
    ...
    if ($this->exists('panels')) :
        ...
        if ($buttonsBlock = $this->fetch('buttons')) :
            ...
            echo $this->element('Atcmobapp/Core.admin/buttons', ['type' => $modelClass]);
