...
$i18nDomain = $this->getRequest()->getParam('plugin') ?: 'atcmobile';
...
if (empty($this->fetch('action-buttons'))) {
    ...
    $this->assign('action-buttons', $this->Atcmobapp->adminAction(__d('atcmobile', 'New %s', __d('atcmobile', Inflector::singularize($this->name))), ['action' => 'add'], ['button' => 'success']));
}
?>
                ...
                if ($mainBlock = trim($this->fetch('main'))) :
                    ...
                    <ul class="nav nav-tabs">
                        ...
                        if ($tabHeading = $this->fetch('tab-heading')) :
                            ...
                            echo $this->Atcmobapp->adminTab(__d('atcmobile', $modelClass), "#$tabId");
                        endif;
                    ...
                    if (empty($tabContent) && !empty($content)) :
                        ...
                        $tabContent .= $this->Atcmobapp->adminTabs();
                    endif;
