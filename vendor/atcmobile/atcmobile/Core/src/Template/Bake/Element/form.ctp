...
if ($action == 'edit'):
    ...
    $this->Breadcrumbs->add(__d('atcmobile', 'Add'), $this->getRequest()->here());
endif;

$this->append('action-buttons');
<% if (strpos($action, 'add') === false): %>
    echo $this->Atcmobapp->adminAction(__('Delete'),
        ['action' => 'delete', $<%= $singularVar %>-><%= $primaryKey[0] %>],
        ['confirm' => __('Are you sure you want to delete # {0}?', $<%= $singularVar %>-><%= $primaryKey[0] %>)]
    );
<% endif; %>
    echo $this->Atcmobapp->adminAction(__('List <%= $pluralHumanName %>'),
        ['action' => 'index']
    );
<%
    $done = [];
...
$this->append('tab-heading');
    echo $this->Atcmobapp->adminTab('<%= $singularHumanName %>', '#<%= $primaryTab %>');
$this->end();
