...
use Atcmobapp\Settings\Configure\Engine\DatabaseConfig;
...
class PluginManager extends Plugin
{
    ...
    public function getPlugins($type = 'plugin')
    {
        ...
        unset($pluginPaths['Atcmobapp']); //Otherwise we get atcmobile plugins twice!
        foreach ($pluginPaths as $pluginName => $pluginPath) {
            ...
            if ((
                    $type === 'plugin' && $this->_isAtcmobappPlugin($pluginPath)
                ) || (
                    $type === 'theme' && $this->_isAtcmobappTheme($pluginPath)
                )
            ) {
            ...
            foreach ($pluginFolders[0] as $pluginFolder) {
                if (substr($pluginFolder, 0, 1) != '.') {
                    if ($type === 'plugin' && !$this->_isAtcmobappPlugin($pluginPath, $pluginFolder)) {
                        continue;
                    }
                    if ($type === 'theme' && !$this->_isAtcmobappTheme($pluginPath, $pluginFolder)) {
                        continue;
                    }
    ...
    /**
     * Checks wether $pluginDir/$path is a Atcmobapp theme
     *
     * @param string $pluginDir plugin directory
     * @param string $path plugin alias
     * @return bool true if path is a Atcmobapp plugin
     */
    protected function _isAtcmobappTheme($pluginDir, $path = '')
    {
        ...
        if (file_exists($composerFile)) {
            ...
            if (property_exists($json, 'type') && $json->type === 'atcmobile-theme') {
                return true;
            }
        }
    ...
    /**
     * Checks wether $pluginDir/$path is a Atcmobapp plugin
     *
     * @param string $pluginDir plugin directory
     * @param string $path plugin alias
     * @return bool true if path is a Atcmobapp plugin
     */
    protected function _isAtcmobappPlugin($pluginDir, $path = '')
    {
        $dir = $pluginDir . $path . DS;
        $composerFile = $dir . 'composer.json';
        ...
        if (file_exists($composerFile) && !$this->_isAtcmobappTheme($pluginDir, $path)) {
            $pluginData = json_decode(file_get_contents($composerFile), true);
            if (isset($pluginData['require']['atcmobile/core']) ||
                isset($pluginData['require']['atcmobile/atcmobile'])
            ) {
                return true;
            }
        }
    ...
    protected function _loadData($alias, $pluginPath, $ignoreMigration = true)
    {
        ...
        if ($hasManifest || $hasComposer) {
            ...
            if ($hasComposer) {
                ...
                $isAtcmobappPlugin = isset($composerData['require']['Atcmobapp/Core']) || $type == 'atcmobile-plugin';

                if ($isAtcmobappPlugin) {
                    if (isset($composerData['name'])) {
            ...
            if (!$ignoreMigration && $this->needMigration($alias, $active)) {
                $pluginData = [
                    'name' => $alias,
                    'description' => "Atcmobapp $alias plugin",
                    'active' => true,
                    'needMigration' => true,
                ];
    ...
    public function activate($plugin, $dependencyList = [])
    {
        ...
        if (!isset($pluginActivation) ||
            ...
            try {
                ...
                return __d('atcmobile', 'Plugin "%s" could not be actived.', $plugin);
            }
    ...
    public function usedBy($plugin)
    {
        ...
        $usedBy = array_filter($deps['usedBy'][$plugin], ['Atcmobapp\\Core\\Plugin', 'loaded']);
        if (!empty($usedBy)) {
            return $usedBy;
        }
    ...
    {
        if (!Plugin::isLoaded($plugin)) {
            return __d('atcmobile', 'Plugin "%s" is not active.', $plugin);
        }
        $pluginActivation = $this->getActivator($plugin);
        if (!isset($pluginActivation) ||
            (isset($pluginActivation) &&
            ...
            return __d('atcmobile', 'Plugin could not be deactivated. Please, try again.');
        }
    }
    ...
    {
        if (empty($plugin)) {
            throw new InvalidArgumentException(__d('atcmobile', 'Invalid plugin'));
        }
    ...
    protected function _move($dir, $plugin, $bootstraps)
    {
        ...
        if ($dir === 'up') {
            ...
            if (in_array($plugin, $dependencies)) {
                return __d('atcmobile', 'Plugin %s depends on %s', $swap, $plugin);
            }
            $reordered = array_merge($before, (array)$swap, (array)$plugin);
        } else {
            return __d('atcmobile', 'Invalid direction');
        }
        $reordered = array_merge($reordered, $after);

        if ($this->_isBuiltin($swap)) {
            return __d('atcmobile', 'Plugin %s cannot be reordered', $swap);
        }
    ...
    protected function _saveBootstraps($bootstraps)
    {
        ...
        if (empty($Setting)) {
            if (!Configure::read('Atcmobapp.installed')) {
                throw new Exception('Unable to save Hook.bootstraps when Atcmobapp is not fully installed');
            }
    ...
    {
        if (strstr($plugin, 'Atcmobapp/')) {
            return realpath(parent::path('Atcmobapp/Core') . '..' . DS . substr($plugin, 7) . DS) . DS;
        }
    ...
    public static function setup(PluginApplicationInterface $app)
    {
        ...
        Type::map('params', 'Atcmobapp\Core\Database\Type\ParamsType');
        Type::map('encoded', 'Atcmobapp\Core\Database\Type\EncodedType');
        Type::map('link', 'Atcmobapp\Core\Database\Type\LinkType');
        ...
        Configure::write('Atcmobapp.Cache.defaultEngine', $defaultEngine);
        Configure::write('Atcmobapp.Cache.defaultPrefix', $defaultPrefix);
        Configure::write('Atcmobapp.Cache.defaultConfig', $cacheConfig);
         ...
         */
        Configure::write('Site.acl_plugin', 'Atcmobapp/Acl');

        /**
         * Default API Route Prefix. This can be overriden in settings.
         */
        Configure::write('Atcmobapp.Api.path', 'api');

        /**
         * Admin theme
         */
        Configure::write('Site.admin_theme', 'Atcmobapp/Core');
     ...
     * @return void
     */
    public static function atcmobileBootstrap($app)
    {
        Configure::write(
            'DebugKit.panels',
            array_merge((array)Configure::read('DebugKit.panels'), [
                'Atcmobapp/Core.Plugins',
                'Atcmobapp/Core.ViewHelpers',
                'Atcmobapp/Core.Components',
            ])
        );

        Atcmobapp::hookComponent('*', [
            'Atcmobapp' => [
                'className' => 'Atcmobapp/Core.Atcmobapp',
                'priority' => 5
            ]
        ]);
        ...
        Atcmobapp::hookComponent('*', 'Acl.Acl');
        Atcmobapp::hookComponent('*', 'Auth');
        Atcmobapp::hookComponent('*', 'Flash');
        //Atcmobapp::hookComponent('*', 'RequestHandler');
        Atcmobapp::hookComponent('*', 'Atcmobapp/Core.Theme');

        Atcmobapp::hookHelper('*', 'Atcmobapp/Core.Js');
        Atcmobapp::hookHelper('*', 'Atcmobapp/Core.Layout');

        // Make sure that the Atcmobapp event manager is the global one
        ...
        /**
         * Use old translation format for the atcmobile domain
         */
        $siteLocale = Configure::read('App.defaultLocale');
        I18n::config('atcmobile', function ($domain, $locale) {
        ...
        time(function () use ($app) {
             ...
             * Extensions
             */
            $app->addPlugin('Atcmobapp/Extensions', [
                'autoload' => true,
                'bootstrap' => true,
                'routes' => true,
                'events' => true
        ...
        // FIXME DispatcherFactory::add('Atcmobapp/Core.HomePage');
        ...
        $setupFile = ROOT . '/config/atcmobile.php';
        if (file_exists ($setupFile)) {
            require_once $setupFile;
        }

        time(function () {
            Atcmobapp::dispatchEvent('Atcmobapp.bootstrapComplete');
        }, 'event-Atcmobapp.bootstrapComplete', 'Event: Atcmobapp.bootstrapComplete');
    }
}
