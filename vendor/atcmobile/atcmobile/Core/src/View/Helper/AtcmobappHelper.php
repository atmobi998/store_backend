...
use Cake\View\View;
use Atcmobapp\Core\Database\Type\ParamsType;
 ...
 */
class AtcmobappHelper extends Helper
{
     ...
     * ParamsType instance
     *
     * @var Atcmobapp\Core\Database\Type\ParamsType;
     */
    ...
    {
        return $this->_AtcmobappStatus->statuses();
    }
    ...
    public function adminMenus($menus, $options = [], $depth = 0)
    {
        ...
        if (empty($this->Role)) {
            $this->Role = TableRegistry::get('Atcmobapp/Users.Roles');
            $this->Role->addBehavior('Atcmobapp/Core.Aliasable');
        }
            ...
            foreach ($tabs as $title => $tab) {
                ...
                if (!isset($tab['options']['type']) ||
                    ...
                    if ($this->adminTabs) {
                        ...
                        $output .= $this->adminTab(__d('atcmobile', $title), '#' . $domId, $tab['options']['linkOptions']);
                    }
                }
            }
        }
    ...
    {
        $linkChooser = $this->_View->element('Atcmobapp/Core.admin/modal', [
            'id' => 'link-chooser',
            'modalSize' => 'modal-lg'
        ]);
        ...
        return $this->_View->cell('Atcmobapp/Core.Admin/LinkChooser', [$target]);
    }
