...
class LayoutHelper extends Helper
{
     ...
     *
     * Shows atcmobile.js file along with useful information like the applications's basePath, etc.
     *
     * Also merges Configure::read('Js') with the Atcmobapp js variable.
     * So you can set javascript info anywhere like Configure::write('Js.my_var', 'my value'),
     * and you can access it like 'Atcmobapp.my_var' in your javascript.
     *
     * @return string
     */
    public function js()
    {
        $atcmobile = $this->_mergeThemeSettings();
        $request = $this->getView()->getRequest();
        if ($request->getParam('locale')) {
            $atcmobile['basePath'] = Router::url('/' . $request->getParam('locale') . '/');
        } else {
            $atcmobile['basePath'] = Router::url('/');
        }
        $validKeys = [
            'plugin' => null,
            'controller' => null,
        ...
        $atcmobile['params'] = array_intersect_key(
            array_merge($validKeys, $request->getAttribute('params')),
            $validKeys
        );
        if (is_array(Configure::read('Js'))) {
            $atcmobile = Hash::merge($atcmobile, Configure::read('Js'));
        }

        return $this->Html->scriptBlock('var Atcmobapp = ' . json_encode($atcmobile) . ';');
    }

    /**
     * Merge helper and prefix specific settings
     *
     * @param array $atcmobileSetting Atcmobapp JS settings
     * @return array Merged settings
     */
    protected function _mergeThemeSettings($atcmobileSetting = [])
    {
        $themeSettings = $this->Theme->settings();
        if (empty($themeSettings)) {
            return $atcmobileSetting;
        }
        $validKeys = [
            'css' => null,
            'icons' => null,
        ...
        $atcmobileSetting['themeSettings'] = array_intersect_key(
        ...
        if ($this->_View->helpers()->has('Html') && $this->_View->helpers()->Html instanceof HtmlHelper) {
            unset($validKeys['css']);
            $atcmobileSetting['themeSettings'] = Hash::merge(
                $atcmobileSetting['themeSettings'],
                array_intersect_key(
    ...
    public function status($value)
    {
        ...
        if (method_exists($this->Html, 'icon')) {
            ...
            if (empty($this->_View->Html)) {
                $this->_View->Helpers->load('Atcmobapp/Core.Html');
            }
    ...
    {
        Atcmobapp::dispatchEvent('Helper.Layout.beforeFilter', $this->_View, [
            'content' => &$content,
            'options' => $options,
        ]);
        $content = $this->filterElements($content, $options);
        Atcmobapp::dispatchEvent('Helper.Layout.afterFilter', $this->_View, [
            'content' => &$content,
            'options' => $options,
        ]);
