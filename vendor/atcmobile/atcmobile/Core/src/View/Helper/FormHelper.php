...
class FormHelper extends BaseFormHelper
{
    ...
    public function __construct(View $View, $settings = [])
    {
        $settings = Hash::merge([
            'widgets' => [
                'stringlist' => [
                    'Atcmobapp/Core.StringList',
                    'textarea',
                    'label',
                ],
                'datetime' => ['Atcmobapp/Core.DateTime', 'select'],
                'button' => ['Atcmobapp/Core.Button']
            ],
        ], $settings);
    ...
    protected function _parseOptions($fieldName, $options)
    {
        ...
        if ((array_key_exists('linkChooser', $options)) && ($options['linkChooser'])) {
            $target = '#' . $options['id'];
            $options['append'] = $this->Atcmobapp->linkChooser($target);
        }
     ...
     * @param array $config setting passed to AtcmobappFormHelper::autocomplete()
