...
class BulkProcessBehavior extends Behavior
{
    ...
    public function processAction($action, $ids)
    {
        ...
        if (empty($actionsMap[$action])) {
            throw new InvalidArgumentException(__d('atcmobile', 'Invalid action to perform'));
        }

        if (empty($ids)) {
            throw new InvalidArgumentException(__d('atcmobile', 'No target to process action upon'));
        }

        $mappedAction = $actionsMap[$action];

        if ($mappedAction === false) {
            throw new InvalidArgumentException(__d('atcmobile', 'Action %s is disabled'), $action);
        }
    ...
    public function bulkCopy($ids)
    {
        if (!$this->_table->hasBehavior('Copyable')) {
            $this->_table->addBehavior('Atcmobapp/Core.Copyable');
        }
