...
Admin.protectForms = function () {
  ...
  if (forms.length > 0) {
    ...
    for (var i = 0; i < forms.length; i++) {
      ...
      $form
        ...
        .on('click', whitelist, function (e) {
          ...
          if (typeof Atcmobapp.Wysiwyg !== 'undefined' && typeof Atcmobapp.Wysiwyg.resetDirty == 'function') {
            Atcmobapp.Wysiwyg.resetDirty();
          }
        });
    }
    ...
    window.onbeforeunload = function (e) {
      ...
      if (!dirty) {
        if (typeof Atcmobapp.Wysiwyg !== 'undefined' && typeof Atcmobapp.Wysiwyg.isDirty == 'function' && !Atcmobapp.Wysiwyg.isDirty()) {
          return;
        } else {
          return;
        }
...
Admin.extra = function () {
  ...
  if (typeof $.fn.select2 !== 'undefined') {
    $('select:not(".no-select2")').select2(Atcmobapp.themeSettings.select2Defaults);
  }
};
...
Admin.iconClass = function (icon, includeDefault) {
  var result = '';
  if (typeof Atcmobapp.themeSettings.icons[icon] === 'string') {
    icon = Atcmobapp.themeSettings.icons[icon];
  }
  if (typeof includeDefault === 'undefined') {
    includeDefault = true;
  }
  if (includeDefault) {
    result = Atcmobapp.themeSettings.iconDefaults['iconSet'] + ' ';
  }
  result += Atcmobapp.themeSettings.iconDefaults['prefix'] + '-' + icon;
  return result.trim();
};
