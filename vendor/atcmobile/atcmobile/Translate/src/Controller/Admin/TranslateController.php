...
namespace Atcmobapp\Translate\Controller\Admin;
 ...
 * @package  Atcmobapp.Translate.Controller
 * @version  1.0
...
class TranslateController extends AppController
{
    ...
    public function initialize()
    {
        ...
        $this->loadModel('Atcmobapp/Settings.Languages');
    }
    ...
    public function index()
    {
        ...
        if ($id == null) {
            $this->Flash->error(__d('atcmobile', 'Invalid ID.'));
        ...
        if (!is_array($config)) {
            $this->Flash->error(__d('atcmobile', 'Invalid model.'));
        ...
        $record = $Model->get($id);
        if (!isset($record->id)) {
            $this->Flash->error(__d('atcmobile', 'Invalid record.'));
        ...
        $this->set('title_for_layout', sprintf(__d('atcmobile', 'Translations: %s'), $record->get($displayField)));
    ...
    public function edit($id = null)
    {
        ...
        if (!$language->id) {
            $this->Flash->error(__d('atcmobile', 'Invalid Language'));
        ...
        if (!empty($this->getRequest()->data)) {
            ...
            if ($Model->save($entity)) {
                $this->Flash->success(__d('atcmobile', 'Record has been translated'));
                $redirect = [
                ...
                $this->Flash->error(__d('atcmobile', 'Record could not be translated. Please, try again.'));
            }
        }
        $this->set(compact(
            'entity',
    ...
    public function delete($id = null, $modelAlias = null, $locale = null)
    {
        if ($locale == null || $id == null) {
            $this->Flash->error(__d('atcmobile', 'Invalid Locale or ID'));
        ...
        if ($Model->deleteTranslation($record, $locale)) {
            $this->Flash->success(__d('atcmobile', 'Translation for the locale deleted successfully.'));
        } else {
            $this->Flash->error(__d('atcmobile', 'Translation for the locale could not be deleted.'));
        }
