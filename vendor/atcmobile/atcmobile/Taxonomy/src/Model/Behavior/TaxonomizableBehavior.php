...
namespace Atcmobapp\Taxonomy\Model\Behavior;
...
use Atcmobapp\Taxonomy\Model\Entity\Term;

/**
 * TaxonomizableBehavior
 *
 * @category Taxonomy.Model.Behavior
 * @package  Atcmobapp.Taxonomy.Model.Behavior
 * @version  1.0
...
//        $events['Model.Node.beforeSaveNode'] = 'beforeSaveNode';
    ...
    protected function _setupRelationships()
    {
        $this->_table->belongsTo('Types', [
            'className' => 'Atcmobapp/Taxonomy.Types',
            'foreignKey' => 'type',
            'bindingKey' => 'alias',
            'propertyName' => 'node_type',
        ]);
        $this->_table->belongsToMany('Taxonomies', [
            'className' => 'Atcmobapp/Taxonomy.Taxonomies',
            'through' => 'Atcmobapp/Taxonomy.ModelTaxonomies',
    ...
    public function validateTaxonomyData(Entity $entity)
    {
        ...
        $requiredError = __d('atcmobile', 'Please select at least 1 value');
        $multipleError = __d('atcmobile', 'Please select at most 1 value');
    ...
    public function formatTaxonomyData(Entity $entity, $typeAlias)
    {
        $type = $this->_table->Taxonomies->Vocabularies->Types->findByAlias($typeAlias)
            ->first();
        if (empty($type)) {
            throw new RecordNotFoundException(__d('atcmobile', 'Invalid Content Type'));
        }
     ...
     * @return \Atcmobapp\Taxonomy\Model\Entity\Term
     */
    ...
    public function findWithTerm(Query $query, array $options)
    {
        ...
        if (!$entity) {
            throw new RecordNotFoundException(__d('atcmobile', 'Term not found: %s', $term));
        }
    ...
    public function findWithVocabulary(Query $query, array $options)
    {
        ...
        if (is_string($vocab)) {
            ...
            $entity = $this->_table->Taxonomies->Vocabularies->find()
                ...
                ->cache($cacheKey, 'atcmobile_vocabularies')
                ->first();
        }

        if (!$entity) {
            throw new RecordNotFoundException(__d('atcmobile', 'Vocabulary not found: {0}', $vocab));
        }
