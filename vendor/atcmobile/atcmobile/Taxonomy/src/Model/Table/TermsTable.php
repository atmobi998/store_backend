 ...
 */
class TermsTable extends AtcmobappTable
{
    ...
    public function initialize(array $config)
    {
        ...
        $this->belongsToMany('Atcmobapp/Taxonomy.Vocabularies', [
            'through' => 'Atcmobapp/Taxonomy.Taxonomies',
            'foreignKey' => 'term_id',
    ...
    public function validationDefault(Validator $validator)
    {
        $validator
            ...
            ->notBlank('slug', __d('atcmobile', 'The slug cannot be empty'))
            ->add('slug', [
                'unique' => [
                    'rule' => 'validateUnique',
                    'provider' => 'table',
                    'message' => __d('atcmobile', 'This slug has already been taken.')
                ]
            ]);
    ...
    public function findByVocabulary(Query $query, array $options)
    {
        if (empty($options['vocabulary_id'])) {
            trigger_error(__d('atcmobile', '"vocabulary_id" key not found'));
        }
