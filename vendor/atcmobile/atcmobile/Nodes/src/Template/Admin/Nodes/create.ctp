<?php
$this->assign('title', __d('atcmobile', 'Create content'));
?>
<h2 class="d-md-none"><?= $this->fetch('title') ?></h2>
<?php
$this->Breadcrumbs
    ->add(__d('atcmobile', 'Content'), ['action' => 'index'])
    ->add(__d('atcmobile', 'Create'), $this->getRequest()->getRequestTarget());

?>
<div class="<?= $this->Theme->getCssClass('row') ?>">
    <div class="<?= $this->Theme->getCssClass('columnFull') ?>">
        <p class="lead"><?= __d('atcmobile', 'Select the type of content you wish to create from the list below') ?></p>
