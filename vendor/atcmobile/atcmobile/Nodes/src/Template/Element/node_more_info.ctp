...
if (!empty($this->Nodes->node->taxonomies)) {
    echo __d('atcmobile', 'Posted in') . ' ' . implode(', ', $nodeTerms);
}
...
if ($this->Nodes->commentsEnabled() && $this->getRequest()->getParam('action') !== 'view' && $type->comment_status) {
    ...
    if ($this->Nodes->field('comment_count') == 0) {
        $commentCount = __d('atcmobile', 'Leave a comment');
    } elseif ($this->Nodes->field('comment_count') == 1) {
        $commentCount = $this->Nodes->field('comment_count') . ' ' . __d('atcmobile', 'Comment');
    } else {
        $commentCount = $this->Nodes->field('comment_count') . ' ' . __d('atcmobile', 'Comments');
    }
