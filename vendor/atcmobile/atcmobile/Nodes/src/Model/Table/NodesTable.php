...
namespace Atcmobapp\Nodes\Model\Table;
...
class NodesTable extends AtcmobappTable
{
    public function initialize(array $config)
    {
        ...
        $this->addBehavior('Atcmobapp/Core.Url', [
            'url' => [
        ...
        $this->addBehavior('Atcmobapp/Core.Copyable', [
            'stripFields' => [
                'model_taxonomies',
            ],
        ]);

        $this->belongsTo('Atcmobapp/Users.Users');
    ...
    {
        $validator->notBlank('title', __d('atcmobile', 'Please supply a title.'));

        $validator->minLength('slug', 1, __d('atcmobile', 'Slug cannot be empty.'));
    ...
    {
        $rules->isUnique(['slug', 'type'], __d('atcmobile', 'The slug has already been taken.'));
        $rules->add(function (Node $node) {
            ...
            return (bool)TableRegistry::get('Atcmobapp/Taxonomy.Types')
    ...
    public function saveNode(Node $node, $typeAlias = self::DEFAULT_TYPE)
    {
        ...
        $event = Atcmobapp::dispatchEvent('Model.Nodes.beforeSaveNode', $this, compact('node', 'typeAlias'));
        if ($event->isStopped()) {
            return $event->result;
        }

        $result = $this->save($node);
        Atcmobapp::dispatchEvent('Model.Nodes.afterSaveNode', $this, $event->data);
    ...
    public function beforeSave(Event $event)
    {
        ...
        $event = Atcmobapp::dispatchEvent('Model.Nodes.beforeSaveNode', $this, [
            'node' => $node,
            'typeAlias' => $node->type
        ]);
        if ($event->isStopped()) {
    ...
    {
        $node = $event->getData()['entity'];
        $event = Atcmobapp::dispatchEvent('Model.Nodes.afterSaveNode', $this, [
            'node' => $node,
            'typeAlias' => $node->type
        ]);
        if ($event->isStopped()) {
