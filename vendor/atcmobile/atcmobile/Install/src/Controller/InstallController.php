...
namespace Atcmobapp\Install\Controller;
...
class InstallController extends Controller
{
    ...
    /** @var \Atcmobapp\Install\InstallManager */
    ...
    public function beforeFilter(Event $event)
    {
        ...
        $this->viewBuilder()->setTheme('Atcmobapp/Core');
        $this->viewBuilder()->setLayout('install');
        $this->viewBuilder()->setClassName('Atcmobapp/Core.Atcmobapp');
    ...
    protected function _check()
    {
        ...
        if (Configure::read('Atcmobapp.installed') && Configure::read('Install.secured')) {
    ...
    public function data()
    {
        ...
        if (!empty($schemaCollection->listTables())) {
            $this->Flash->error(
                __d('atcmobile', 'Warning: Database "%s" is not empty.', $connection->config()['database'])
            );
        ...
        if ($result !== true) {
            $this->Flash->error($result === false ? __d('atcmobile', 'There was a problem installing Atcmobapp') : $result);
    ...
    public function acl()
    {
        try {
            ...
            $this->Flash->error(__d('atcmobile', 'Error installing access control objects'));
    ...
    {
        $this->_check();
        if (!Plugin::isLoaded('Atcmobapp/Users')) {
            Plugin::load('Atcmobapp/Users');
        }
