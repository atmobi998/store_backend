...
namespace Atcmobapp\Comments\Controller;
...
class CommentsController extends AppController
{
    ...
    {
        $this->set('title_for_layout', __d('atcmobile', 'Comments'));

        if ($this->request->getParam('_ext') != 'rss') {
            return $this->redirect('/');
        }

        $roleId = $this->Atcmobapp->roleId();
        $this->paginate = [
            'contain' => ['Nodes', 'Users'],
            'conditions' => [
    ...
    public function add($model = null, $foreignKey = null, $parentId = null)
    {
        ...
        if (!is_null($parentId) && !$this->Comments->isValidLevel($parentId)) {
            $this->Flash->error(__d('atcmobile', 'Maximum level reached. You cannot reply to that comment.'));
        ...
        if (!$continue) {
            $this->Flash->error(__d('atcmobile', 'Comments are not allowed.'));
        ...
        if (!empty($this->getRequest()->data) && $continue === true) {
            ...
            try {
                ...
                $this->Flash->error(__d('atcmobile', 'There was an error when posting your comment'));
            }
            if ($success) {
                if ($autoApprove) {
                    $messageFlash = __d('atcmobile', 'Your comment has been added successfully.');
                } else {
                    $messageFlash = __d('atcmobile', 'Your comment will appear after moderation.');
                }
    ...
    protected function _spamProtection($continue, $spamProtection, $node)
    {
        ...
        if (!empty($body) &&
            ...
            if ($this->Akismet->isCommentSpam()) {
                $continue = false;
                $this->Flash->error(__d('atcmobile', 'Sorry, the comment appears to be spam.'));
            }
        }
    ...
    protected function _captcha($continue, $captchaProtection, $node)
    {
        if (!empty($this->getRequest()->data) &&
            ...
            $continue = false;
            $this->Flash->error(__d('atcmobile', 'Sorry, the comment did not pass the security challenge'));
        }
