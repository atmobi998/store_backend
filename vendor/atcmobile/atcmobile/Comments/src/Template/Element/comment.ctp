...
<div id="comment-<?= $comment->id; ?>" class="comment-level-<?= $level ?> comment<?php if (isset($entity) && $entity->user_id == $comment->user_id):
    ...
    <div class="comment-info">
        ...
        <span class="date"><?= h(__d('atcmobile', 'said on %s', $this->Time->i18nFormat($comment->created))); ?></span>
    </div>
    ...
    <div class="comment-reply">
        <?php if ($level <= Configure::read('Comment.level')) : ?>
            <?= $this->Html->link(__d('atcmobile', 'Reply'), [
    ...
    <?php foreach ((array)$comment->children as $childComment) : ?>
        <?= $this->element('Atcmobapp/Comments.comment', ['comment' => $childComment, 'level' => $level + 1]); ?>
    <?php endforeach; ?>
</div>
