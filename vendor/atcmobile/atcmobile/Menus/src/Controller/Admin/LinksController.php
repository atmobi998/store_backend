...
use Atcmobapp\Core\Controller\AtcmobappAppController;
use Atcmobapp\Menus\Controller\MenusAppController;
use Atcmobapp\Menus\Model\Table\LinksTable;
...
class LinksController extends AppController
{
    ...
    public function index()
    {
        ...
        $this->set('title_for_layout', __d('atcmobile', 'Links: %s', $menu->title));
    ...
    public function delete($id = null)
    {
        ...
        $this->Flash->success(__d('atcmobile', 'Link deleted'));
    ...
    public function moveup($id, $step = 1)
    {
        try {
            ...
            $this->Flash->error(__d('atcmobile', 'Invalid id for Link'));
        ...
        if ($this->Links->moveUp($link, $step)) {
            ...
            $this->Flash->success(__d('atcmobile', 'Moved up successfully'));
        } else {
            $this->Flash->error(__d('atcmobile', 'Could not move up'));
        }
    ...
    public function movedown($id, $step = 1)
    {
        ...
        if ($this->Links->moveDown($link, $step)) {
            Cache::clearGroup('menus', 'atcmobile_menus');
            $this->Flash->success(__d('atcmobile', 'Moved down successfully'));
        } else {
            $this->Flash->error(__d('atcmobile', 'Could not move down'));
        }
    ...
    public function process($menuId = null)
    {
        ...
        $multiple = ['copy' => false];
        $messageMap = [
            'delete' => __d('atcmobile', 'Links deleted'),
            'publish' => __d('atcmobile', 'Links published'),
            'unpublish' => __d('atcmobile', 'Links unpublished'),
        ];
