 ...
 */
class SettingsTable extends AtcmobappTable
{

    public function validationDefault(Validator $validator)
    {
        $validator
            ->notBlank('key', __d('atcmobile', 'Key cannot be empty.'));
    ...
    public function buildRules(RulesChecker $rules)
    {
        $rules
            ->add($rules->isUnique(
                ['key'],
                __d('atcmobile', 'That key is already taken')
            ));
...
//        $this->addBehavior('Atcmobapp/Core.Ordered', [
//            'field' => 'weight',
//            'foreign_key' => false,
//        ]);
    ...
    {
        $gitDir = realpath(PluginManager::path('Atcmobapp/Core') . '..') . DS . '.git';
        ...
        if ($version) {
            return $this->write('Atcmobapp.version', $version);
        }
    }
    ...
    public function updateAppVersionInfo()
    {
        ...
        $version = trim(shell_exec('git describe --tags'));
        if ($version) {
            return $this->write('Atcmobapp.appVersion', $version);
        }
    }
