 ...
 */
class MobrescafsController extends AppController
{
    ...
    public function beforeFilter(Event $event)
    {
        ...
        $this->Auth->allow([ 'usrtoken', 'postoken', 'addsess', 'editsess', 'sessqry', 'addprod', 'editprod', 'prodqry', 
                            'addcat', 'delcat', 'editcat', 'catqry', 'orddetadd', 'orddetdel', 'orddetedit', 'orddetqry', 
                            'ordadd', 'ordedit', 'ordqry', 'produpd' ]);
    }

    public function produpd()
    {
        ...
        if ($token) {
            ...
            if ($struser) {
                ...
                if ($this->Mobprods->exists(['id' => $ajdata['id']])) {
                    $thisprod = $this->Mobprods->get($ajdata['id']);
                    if ($ajdata['qrymode'] == 'stkin' && $ajdata['quantity'] > 0 && $thisprod->store_id == $ajdata['store_id']) {
                        $savedprod = $thisprod->toArray();
                        $thisprod->stockunits += $ajdata['quantity'];
                        $this->Mobprods->save($thisprod);
                        $savedprod = $this->Mobprods->get($ajdata['id'])->toArray();
                        $return_code=['status' => 1, 'data' => $savedprod, 'error' => ''];
                        $initinv = $this->Mobprodinvs->newEntity();
                        $initinv->store_id = $ajdata['store_id'];
                        $initinv->prod_id = $thisprod->id;
                        $initinv->prod_code = $thisprod->prod_code;
                        $initinv->quantity = $ajdata['quantity'];
                        $initinv->note = $ajdata['stkcomment'];
                        if ($ajdata['inv_id']>0) {
                            $initinv->inv = 1;
                            $initinv->inv_id = $ajdata['inv_id'];
                        }
                        if ($ajdata['kitchen_id']>0) {
                            $initinv->kitchen = 1;
                            $initinv->kitchen_id = $ajdata['kitchen_id'];
                        }
                        if ($ajdata['front_id']>0) {
                            $initinv->front = 1;
                            $initinv->front_id = $ajdata['front_id'];
                        }
                        if ($ajdata['pos_id']>0) {
                            $initinv->pos = 1;
                            $initinv->pos_id = $ajdata['pos_id'];
                        }
                        $this->Mobprodinvs->save($initinv);
                    }
                    if ($ajdata['qrymode'] == 'stkout' && $ajdata['quantity'] > 0 && $thisprod->store_id == $ajdata['store_id']) {
                        $savedprod = $thisprod->toArray();
                        $thisprod->stockunits -= $ajdata['quantity'];
                        $this->Mobprods->save($thisprod);
                        $savedprod = $this->Mobprods->get($ajdata['id'])->toArray();
                        ...
                        $initinv->prod_id = $thisprod->id;
                        $initinv->prod_code = $thisprod->prod_code;
                        $initinv->quantity = 0 - $ajdata['quantity'];
                        $initinv->note = $ajdata['stkcomment'];
                        if ($ajdata['inv_id']>0) {
                            $initinv->inv = 1;
                            $initinv->inv_id = $ajdata['inv_id'];
    ...
    public function ordadd()
    {
        ...
        if ($token) {
            ...
            if ($struser) {
                ...
                $theuser = ($this->Profiles->exists(['id' => $thestore['user_id']]))? $this->Profiles->get($thestore['user_id'])->toArray() : array();
                unset($ajdata['id']);
                $initord = $this->Mobords->newEntity();
                $ajdata['modified'] = $ajdata['time'];
                $this->Mobords->patchEntity($initord,$ajdata);$this->Mobords->save($initord);
                $savedord = $this->Mobords->get($initord->id)->toArray();
                $savedord['detail'] = array();
                $return_code=['status' => 1, 'data' => $savedord, 'error' => ''];
    ...
    public function ordedit()
    {
        ...
        if ($token) {
            ...
            if ($struser) {
                $thestore = ($this->Mobstores->exists(['id' => $struser['store_id']]))? $this->Mobstores->get($struser['store_id'])->toArray() : array();
                $theuser = ($this->Profiles->exists(['id' => $thestore['user_id']]))? $this->Profiles->get($thestore['user_id'])->toArray() : array();
                if ($this->Mobords->exists(['id' => $ajdata['id']]) && $ajdata['qrymode'] == 'checkout') {
                    $thisord = $this->Mobords->get($ajdata['id']);
                    $this->Mobords->patchEntity($thisord,['rescaf_checkout' => $ajdata['rescaf_checkout'],
                                                            'rescafcheckout_log' => $ajdata['rescafcheckout_log'], 
                                                            'status' => $ajdata['status'], 'modified' => $ajdata['time'], 
                                                        ]);
                    $this->Mobords->save($thisord);
                    $thisord = $this->Mobords->get($ajdata['id'])->toArray();
                    $return_code=['status' => 1, 'data' => $thisord , 'error' => ''];
                }
                if ($this->Mobords->exists(['id' => $ajdata['id']]) && $ajdata['qrymode'] == 'paid') {
                    $thisord = $this->Mobords->get($ajdata['id']);
                    $this->Mobords->patchEntity($thisord,['rescaf_paid' => $ajdata['rescaf_paid'],
                                                            'rescafpaid_log' => $ajdata['rescafpaid_log'], 
                                                            'status' => $ajdata['status'], 'modified' => $ajdata['time'], 
                                                        ]);
                    $this->Mobords->save($thisord);
                    $thisord = $this->Mobords->get($ajdata['id'])->toArray();
    ...
    public function ordqry()
    {
        ...
        if ($token) {
            ...
            if ($struser) {
                $thestore = ($this->Mobstores->exists(['id' => $struser['store_id']]))? $this->Mobstores->get($struser['store_id'])->toArray() : array();
                $theuser = ($this->Profiles->exists(['id' => $thestore['user_id']]))? $this->Profiles->get($thestore['user_id'])->toArray() : array();
                if (!empty($ajdata['qrymode']) && $ajdata['qrymode'] == 'all_by_store' && !empty($ajdata['store_id'] && $ajdata['store_id'] == $thestore['id'])) {
                    $allords=$this->Mobords->find('all',array('recursive'=>0,'limit' => 9999,
                                                    'fields'=>$this->FldsMobords,
                                                    'order'=>array('Mobords.rescaf_tab DESC'), 
                                                    'conditions'=>array('AND'=>array('Mobords.store_id' => $ajdata['store_id'],'Mobords.rescaf' => 1,'Mobords.rescaf_paid' => 0))))->toArray();
                    foreach ($allords as $ordk => $ordv) {
                        $allords[$ordk]['detail'] = [];
                        $allordprds=$this->Mobordprds->find('all',array('recursive'=>0,'limit' => 9999,'order'=>array('Mobordprds.created'), 
                        'fields'=>$this->FldsMobordprds,
                        'conditions'=>array('AND'=>array('Mobordprds.order_id' => $allords[$ordk]['id']))));
                        if ($allordprds->count() > 0) {
                            $allords[$ordk]['detail'] = $allordprds->toArray();
                        }
                    }
                    $return_code=['status' => 1, 'data' => $allords, 'error' => ''];
                }
                if (!empty($ajdata['qrymode']) && $ajdata['qrymode'] == 'all_not_counted' && !empty($ajdata['store_id'] && $ajdata['store_id'] == $thestore['id'])) {
                    $allords=$this->Mobords->find('all',array('recursive'=>0,'limit' => 9999,
                                                    'fields'=>$this->FldsMobords,
                                                    'order'=>array('Mobords.rescaf_tab DESC'), 
                                                    'conditions'=>array('AND'=>array('Mobords.store_id' => $ajdata['store_id'],
                                                                                    'Mobords.rescaf' => 1,'Mobords.rescaf_paid' => 1,
                                                                                    'Mobords.rescaf_counted' => 0,
                                                                                    ))))->toArray();
                    foreach ($allords as $ordk => $ordv) {
                        $allords[$ordk]['detail'] = [];
                        $allordprds=$this->Mobordprds->find('all',array('recursive'=>0,'limit' => 9999,'order'=>array('Mobordprds.created'), 
                ...
                if (!empty($ajdata['qrymode']) && $ajdata['qrymode'] == 'pos_transfer' && !empty($ajdata['store_id']) && $ajdata['store_id'] == $thestore['id'] && !empty($ajdata['pos_id'])) {
                    $initrescafcnt = $this->Rescafcnts->newEntity();
                    $this->Rescafcnts->patchEntity($initrescafcnt,['store_id' => $ajdata['store_id'], 'store_name' => $ajdata['store_name'],
                                                            'pos_id' => $ajdata['pos_id'], 'sess_id' => $ajdata['sess_id'], 
                                                            'total' => $ajdata['total'], 'subtotal' => $ajdata['subtotal'], 
                                                            'tax' => $ajdata['tax'], 'orders' => $ajdata['orders'], 
                                                            'pos_name' => $struser['name'], 'pos_username' => $struser['usr_name'], 
                                                            'order_count' => $ajdata['order_count'], 'ip_address' => $this->getRequest()->clientIp(), 
                                                            'created' => $ajdata['created'], 
                                                        ]);
                    $this->Rescafcnts->save($initrescafcnt);
                    $orders = explode(" ", $ajdata['orders']);
                    $allords=$this->Mobords->find('all',array('recursive'=>0, 'conditions'=>array('AND'=>array('Mobords.id IN ' => $orders))));
                    foreach ($allords as $thisord) {
                        if ($thisord->rescaf_paid == 1 && $thisord->rescaf_counted == 0) {
                            $thisord->rescaf_counted = 1;
                            $this->Mobords->save($thisord);
                        }
                    }
                    $return_code=['status' => 1, 'data' => $initrescafcnt, 'error' => ''];
                }
            }
        }
        $this->set('return_code', $return_code);
            ...
            if ($struser) {
                if (!empty($ajdata['store_id'])) {
                    ...
                    if ($struser['store_id'] == $ajdata['store_id']) {
                        if (!empty($ajdata['qrymode']) && !empty($ajdata['pos_id'])) {
                            if ($ajdata['qrymode'] == 'allbystore' && !empty($ajdata['store_id']) && $ajdata['store_id'] == $thestore['id'] && !empty($ajdata['pos_id'])) {
                                $allsess=$this->Rescafcnts->find('all',array('recursive'=>0,'limit' => 93,
                                                                                'order'=>array('Rescafcnts.created DESC'), 
                                                                                'fields'=>$this->FldsRescafcnts,
                                                                                'conditions'=>array('AND'=>array('Rescafcnts.store_id' => $thestore['id']))))->toArray();
                                foreach ($allsess as $sk => $sv) {
                                    if (!empty($sv['orders'])) {
                                        $orders = explode(" ", $sv['orders']);
                                        $allords=$this->Mobords->find('all',array('recursive'=>0, 'fields'=>$this->FldsMobords, 'conditions'=>array('AND'=>array('Mobords.id IN ' => $orders))))->toArray();
                                        foreach ($allords as $ok => $ov) {
                                            $allords[$ok]['detail'] = [];
                                            $allordprds=$this->Mobordprds->find('all',array('recursive'=>0,'limit' => 9999,'order'=>array('Mobordprds.created'), 
                                            'fields'=>$this->FldsMobordprds,
                                            'conditions'=>array('AND'=>array('Mobordprds.order_id' => $allords[$ok]['id']))));
                                            if ($allordprds->count() > 0) {
                                                $allords[$ok]['detail'] = $allordprds->toArray();
                                            }
                                        }
                                        $allsess[$sk]['detail'] = $allords;
                                    } else {
                                        $allsess[$sk]['detail'] = [];
                                    }
                                }
    ...
    public function orddetadd()
    {
        ...
        if ($token) {
            ...
            if ($struser) {
                ...
                if (!empty($ajdata['prod_id']) && !empty($ajdata['order_id']) && $ajdata['quantity'] > 0) {
                    $thisprod = ($this->Mobprods->exists(['id' => $ajdata['prod_id']]))? $this->Mobprods->get($ajdata['prod_id'])->toArray() : array();
                    if ($thisprod) {
                        if ($ajdata['qrymode'] == 'new') {
                            $inititm = $this->Mobordprds->newEntity();
                            unset($ajdata['id']);
                        } else if ($ajdata['qrymode'] == 'change') {
                            $inititm = $this->Mobordprds->get($ajdata['id']);
                        }
                        $itemprice = $thisprod['pricesell'];
                        $itemtax = ($thisprod['taxrate'] * $thisprod['pricesell'] * $ajdata['quantity'] / 100);
                        $itemsubtotal = $thisprod['pricesell'] * $ajdata['quantity'] + $itemtax;
                        $newitm = ['order_id' => $ajdata['order_id'], 'prod_id' => $ajdata['prod_id'], 'prod_code' => $thisprod['prod_code'], 
                                    'quantity' => $ajdata['quantity'], 'prd_name' => $thisprod['name'],
                                    'price' => $itemprice, 'tax' => $itemtax,
                                    'subtotal' => $itemsubtotal, 'front_status' => $ajdata['front_status'], 'front_id' => $ajdata['front_id'], 
                                    'kitchen_need' => $ajdata['kitchen_need'], 'kitchen_accept' => 0, 
                                    'inv_need' => $ajdata['inv_need'], 'inv_accept' => 0, 
                                    'rescaf' => $ajdata['rescaf'], 'rescaf_tab' => $ajdata['rescaf_tab'], 'rescaf_take' => $ajdata['rescaf_take'],
                                    'created' => $ajdata['created'], 'modified' => $ajdata['time'],
                                ];
                        
                        $this->Mobordprds->patchEntity($inititm,$newitm);$this->Mobordprds->save($inititm);
                        $allitems=$this->Mobordprds->find('all',array('recursive'=>0,'order'=>array('Mobordprds.id DESC'),'conditions'=>array('AND'=>array('Mobordprds.order_id' => $ajdata['order_id']))))->toArray();
                        $orditems=0;
                        $ordtotal=0;
                        $ordsubtotal=0;
                        $ordtax=0;
                        foreach ($allitems as $ik => $iv) {
                            $orditems = $orditems + $iv['quantity'];
                            $ordsubtotal = $ordsubtotal + $iv['subtotal'];
                            $ordtax = $ordtax + $iv['tax'];
                            $ordtotal = $ordtotal + $iv['subtotal'];
                        }
                        $thisord = $this->Mobords->get($ajdata['order_id']);
                        $this->Mobords->patchEntity($thisord,['total' => $ordtotal,'item_count' => $orditems, 'subtotal' => $ordsubtotal, 'tax' =>$ordtax]);
                        $this->Mobords->save($thisord);

                        $saveddet = $this->Mobordprds->get($inititm->id)->toArray();
    ...
    public function orddetedit()
    {
        ...
        if ($token) {
            ...
            if ($struser) {
                $thestore = ($this->Mobstores->exists(['id' => $struser['store_id']]))? $this->Mobstores->get($struser['store_id'])->toArray() : array();
                $theuser = ($this->Profiles->exists(['id' => $thestore['user_id']]))? $this->Profiles->get($thestore['user_id'])->toArray() : array();
                if (!empty($ajdata['id']) && !empty($ajdata['order_id'])) {
                    $thisorddet = ($this->Mobordprds->exists(['id' => $ajdata['id']]))? $this->Mobordprds->get($ajdata['id']) : false;
                    if ($thisorddet && $ajdata['qrymode'] == 'frontdelivered') {
                        $savedata = [];
                        $savedata['kitchen_done'] = ($thisorddet->kitchen_need > 0)? 1 : 0;
                        $savedata['kitchen_accept'] = ($thisorddet->kitchen_need > 0)? 1 : 0;
                        $savedata['inv_done'] = ($thisorddet->inv_need > 0)? 1 : 0;
                        $savedata['inv_accept'] = ($thisorddet->inv_need > 0)? 1 : 0;
                        $savedata['delivered'] = 1;
                        $savedata['secfront_id'] = $ajdata['secfront_id'];
                        $savedata['secfront_status'] = $ajdata['secfront_status'];
                        $savedata['modified'] = $ajdata['time'];
                        $this->Mobordprds->patchEntity($thisorddet,$savedata);$this->Mobordprds->save($thisorddet);
                        $saveddet = $this->Mobordprds->get($thisorddet->id)->toArray();
                        $return_code=['status' => 1, 'data' => $saveddet, 'error' => ''];   
                    }
                    if ($thisorddet && $ajdata['qrymode'] == 'kitchenaccept') {
                        $savedata = [];
                        $savedata['kitchen_accept'] = ($thisorddet->kitchen_need > 0)? 1 : 0;
                        $savedata['kitchen_id'] = $ajdata['kitchen_id'];
                        $savedata['kitchen_status'] = $ajdata['kitchen_status'];
                        $savedata['modified'] = $ajdata['time'];
                        $this->Mobordprds->patchEntity($thisorddet,$savedata);$this->Mobordprds->save($thisorddet);
                        $saveddet = $this->Mobordprds->get($thisorddet->id)->toArray();
                        $return_code=['status' => 1, 'data' => $saveddet, 'error' => ''];   
                    }
                    if ($thisorddet && $ajdata['qrymode'] == 'kitchendone') {
                        $savedata = [];
                        $savedata['kitchen_done'] = ($thisorddet->kitchen_need > 0)? 1 : 0;
                        $savedata['kitchen_accept'] = ($thisorddet->kitchen_need > 0)? 1 : 0;
                        $savedata['kitchen_id'] = $ajdata['kitchen_id'];
                    ...
                    if ($thisorddet && $ajdata['qrymode'] == 'inventoryaccept') {
                        $savedata = [];
                        $savedata['inv_accept'] = ($thisorddet->inv_need > 0)? 1 : 0;
                        $savedata['inv_id'] = $ajdata['inv_id'];
                        $savedata['inv_status'] = $ajdata['inv_status'];
                        $savedata['modified'] = $ajdata['time'];
                        $this->Mobordprds->patchEntity($thisorddet,$savedata);$this->Mobordprds->save($thisorddet);
                        $saveddet = $this->Mobordprds->get($thisorddet->id)->toArray();
                        $return_code=['status' => 1, 'data' => $saveddet, 'error' => ''];   
                    }
                    if ($thisorddet && $ajdata['qrymode'] == 'inventorydone') {
                        $savedata = [];
                        $savedata['inv_done'] = ($thisorddet->inv_need > 0)? 1 : 0;
                        $savedata['inv_accept'] = ($thisorddet->inv_need > 0)? 1 : 0;
                        $savedata['inv_id'] = $ajdata['inv_id'];
    ...
    public function addcat()
    {
        ...
        if ($token) {
            ...
            if ($struser) {
                $thestore = ($this->Mobstores->exists(['id' => $struser['store_id']]))? $this->Mobstores->get($struser['store_id'])->toArray() : array();
                $user = ($this->Profiles->exists(['id' => $thestore['user_id']]))? $this->Profiles->get($thestore['user_id'])->toArray() : array();
                if (!empty($ajdata)) {
