 ...
 */
class MobprodsController extends AppController
{
    ...
    public function beforeFilter(Event $event)
    {
        ...
        $this->Auth->allow(['addprod', 'delprod', 'editprod', 'getprod', 'prodqry']);
    }
        ...
        if ($token) {
            ...
            if ($user) {
                if (!empty($ajdata['store_id'])) {
                    $savestore = ($this->Mobstores->exists(['id' => $ajdata['store_id']]))? $this->Mobstores->get($ajdata['store_id'])->toArray() : array();
                    if ($savestore['user_id'] == $user['id']) {
                        $initprod = $this->Mobprods->newEntity();
                        $ajdata['user_id'] = $user['id'];
                        $olddata = [];
                        if (empty($ajdata['prod_code']) || ($ajdata['prod_code'] == -1)) {
                            $ajdata['prod_code'] = $ajdata['store_id'] * 2000 + $ajdata['id'];
                        }
                        if (empty($ajdata['prod_sku'])) {
                            $ajdata['prod_sku'] = $ajdata['prod_code'].'_sku';
                        }
                        if (empty($ajdata['prod_ref'])) {
                            $ajdata['prod_ref'] = $ajdata['prod_code'].'_ref#';
                        }					
    ...
    public function editprod()
    {
        ...
        if ($token) {
            $user=$this->verifytoken($token);
            if ($user) {
                if ($ajdata['id'] && !empty($ajdata['store_id'])) {
                    $savestore = ($this->Mobstores->exists(['id' => $ajdata['store_id']]))? $this->Mobstores->get($ajdata['store_id'])->toArray() : array();
                    if ($savestore['user_id'] == $user['id']) {
                        if (empty($ajdata['prod_code']) || ($ajdata['prod_code'] == -1)) {
                            $ajdata['prod_code'] = $ajdata['store_id'] * 2000 + $ajdata['id'];
                        ...
                        if ($ajdata['id'] == -1) {
                            ...
                            $limit_upload_w = 1024;
                            if ($this->Mobprods->save($initprod)) {
                                $product = ($this->Mobprods->exists(['id' => $initprod->id]))? $this->Mobprods->get($initprod->id)->toArray() : array();
                                $ajdata = $product;
                                $ajdata['user_id'] = $user['id'];
                                $olddata = $product;
                            }
        ...
        if ($token) {
            ...
            if ($user) {
                if ($ajdata['qrymode'] == 'allbycat') {
                    $category = ($this->Mobcats->exists(['id' => $ajdata['cat_id']]))? $this->Mobcats->get($ajdata['cat_id'])->toArray() : array();
                    $savestore = ($this->Mobstores->exists(['id' => $category['store_id']]))? $this->Mobstores->get($category['store_id'])->toArray() : array();
                    if ($savestore['user_id'] == $user['id']) {
                        if ($ajdata['filter'] != '') {
                            $allprod=$this->Mobprods->find('all',array('recursive'=>0,'order'=>array('Mobprods.sort ASC'),
                                'fields'=>$this->FldsMobprods,
                            ...
                            $allprod=$this->Mobprods->find('all',array('recursive'=>0,'order'=>array('Mobprods.sort ASC'), 
                                'fields'=>$this->FldsMobprods,
                                'conditions'=>array('AND'=>array('Mobprods.cat_id' => $ajdata['cat_id']))))->toArray();
                            $prodsort=1;
                            foreach ($allprod as $prodk => $prodv) {
                                $theprod = $this->Mobprods->get($prodv['id']);
                                $theprod->sort = $prodsort++;
                                $this->Mobprods->save($theprod);
                            }
                            $allprod=$this->Mobprods->find('all',array('recursive'=>0,'order'=>array('Mobprods.sort ASC'), 
                                'fields'=>$this->FldsMobprods,
                                'conditions'=>array('AND'=>array('Mobprods.cat_id' => $ajdata['cat_id']))))->toArray();
                ...
                if ($ajdata['qrymode'] == 'allbystore') {
                    ...
                    if ($savestore['user_id'] == $user['id']) {
                        $allprod=$this->Mobprods->find('all',array('recursive'=>0,'order'=>array('Mobprods.sort ASC'),
                            'fields'=>$this->FldsMobprods,
                            'conditions'=>array('AND'=>array('Mobprods.store_id' => $ajdata['store_id']))))->toArray();
                        $return_code=['status' => 1, 'data' => $allprod, 'error' => ''];
                    }
                }
