 ...
 */
class MobdrvsController extends AppController
{
    ...
    public function beforeFilter(Event $event)
    {
        ...
        $this->Auth->allow(['login','token','upfund','transqry','balance','adddrv','deldrv','editdrv','getdrv','drvqry','homeloc','drvcab','cabqry',
                            'drvloc','editnid','editpay','editcar','editcarlic','editbike','editbikelic']);
    }

    public function drvloc()
    {
        ...
        $return_code=['status' => 0, 'data' => [], 'curcab' => [], 'cabs' => [], 'cabcount' => 0,'error' => ''];
        $limit_upload_w=1024;
        ...
        if ($token) {
            ...
            if ($user) {
                if ($ajdata['id']) {
                    $savedrv = ($this->Mobdrvs->exists(['id' => $ajdata['id']]))? $this->Mobdrvs->get($ajdata['id'])->toArray() : array();
                    if ($savedrv['user_id'] == $user['id']) {
                        $scancabs = ($ajdata['scan_cabs'] > 0)? true : false;unset($ajdata['scan_cabs']);
                        $olddata = $savedrv;
                        ...
                        if ($ajdata['newcode'] == '1') {
                            $ajdata['drv_code'] = substr(base64_encode(random_bytes(15)), 0, 14);
                        }
                        unset($ajdata['photo_img64']);unset($ajdata['photofile']);
                        $initdrv = $this->Mobdrvs->get($savedrv['id']);
                        if (empty($savedrv['drv_code'])) {
                            $ajdata['drv_code'] = substr(base64_encode(random_bytes(15)), 0, 14);
                        }
                        $this->Mobdrvs->patchEntity($initdrv, $ajdata);
                        if ($this->Mobdrvs->save($initdrv)) {
                            $savedrv = ($this->Mobdrvs->exists(['id' => $savedrv['id']]))? $this->Mobdrvs->get($savedrv['id'])->toArray() : array();
                            $return_code=['status' => 1, 'data' => $savedrv, 'curcab' => [], 'cabs' => [], 'cabcount' => 0, 'error' => ''];
                        }
                        if ($ajdata['cab_id'] > 0) {
                            $cabid=$ajdata['cab_id'];
                            $initcab = $this->Mobcabs->get($cabid);$cabdata=[];
                            $initcabary = $initcab->toArray();
                            if (($initcabary['drv_id'] == 0 && $ajdata['pickcab'] == 1) || ($initcabary['drv_id'] == $savedrv['id'] && $ajdata['pickcab'] == 1)) {
                                $cabdata['drv_id'] = $savedrv['id'];
                                $cabdata['drv_name'] = $savedrv['drv_name'];
                                $cabdata['drv_phone'] = $savedrv['drv_phone'];
                                $cabdata['drv_email'] = $savedrv['drv_email'];
                                $cabdata['drv_img'] = $savedrv['drv_img'];
                                $cabdata['drv_img_w'] = $savedrv['img_w'];
                                $cabdata['drv_img_h'] = $savedrv['img_h'];
                                $cabdata['drv_lat'] = $ajdata['drv_lat'];
                                $cabdata['drv_lng'] = $ajdata['drv_lng'];
                                if ($initcabary['is_car']) {
                                    $cabdata['vmodel'] = $savedrv['ca_vmodel'];
                                    $cabdata['vmanyr'] = $savedrv['ca_vmanyr'];
                                    $cabdata['vseats'] = $savedrv['ca_seats'];
                                    $cabdata['vlicplat'] = $savedrv['ca_vlicplat'];
                                } else {
                                    $cabdata['vmodel'] = $savedrv['bi_vmodel'];
                                    $cabdata['vmanyr'] = $savedrv['bi_vmanyr'];
                                    $cabdata['vseats'] = 1;
                                    $cabdata['vlicplat'] = $savedrv['bi_vlicplat'];
                                }
                                $this->Mobcabs->patchEntity($initcab,$cabdata);
                                if ($this->Mobcabs->save($initcab)) {
                                    $savecab = ($this->Mobcabs->exists(['id' => $cabid]))? $this->Mobcabs->get($cabid)->toArray() : array();
                                    $return_code['curcab'] = $savecab;
                                    $return_code['cabcount'] = 1;
                                }
                            }
                            if ($initcabary['drv_id'] == $savedrv['id'] && $ajdata['pickcab'] == 0) {
                                $cabdata['drv_lat'] = $ajdata['drv_lat'];
                                $cabdata['drv_lng'] = $ajdata['drv_lng'];
                                if (!empty($ajdata['taxi_status'])) {
                                    $cabdata['taxi_status'] = $ajdata['taxi_status'];
                                }
                                if (!empty($ajdata['status'])) {
                                    $cabdata['status'] = $ajdata['status'];
                                }
                                if (!empty($ajdata['cabpickedup'])) {
                                    $cabdata['cabpickedup'] = $ajdata['cabpickedup'];
                                }
                                if (!empty($ajdata['cabfinish'])) {
                                    $cabdata['cabfinish'] = $ajdata['cabfinish'];
                                }
                                $this->Mobcabs->patchEntity($initcab,$cabdata);
                                if ($this->Mobcabs->save($initcab)) {
                                    $savecab = ($this->Mobcabs->exists(['id' => $cabid]))? $this->Mobcabs->get($cabid)->toArray() : array();
                        ...
                        if ($scancabs) {
                            ...
                            $searchradius = 1;
                            $cabslimit = 10;
                            $sphericalCosineSql = "(:earth_radius * ACOS(COS(RADIANS(:latitude)) * COS(RADIANS(Mobcabs.start_lat)) * 
                                COS( RADIANS(Mobcabs.start_lng) - RADIANS(:longitude) ) + 
                                SIN(RADIANS(:latitude)) * SIN(RADIANS(Mobcabs.start_lat))))";
                            $qdistance = "ROUND($sphericalCosineSql, 3)";
                            $queryOptions = [
                                'fields' => ['qdistance' => $qdistance, 'Mobcabs.id', 'Mobcabs.drv_id', 'Mobcabs.drv_name', 'Mobcabs.drv_phone', 'Mobcabs.drv_email', 'Mobcabs.drv_img', 'Mobcabs.drv_img_w', 'Mobcabs.drv_img_h', 'Mobcabs.drv_lat', 'Mobcabs.drv_lng', 
                                            'Mobcabs.vmodel', 'Mobcabs.vmanyr', 'Mobcabs.vseats', 'Mobcabs.vlicplat', 'Mobcabs.cli_id', 'Mobcabs.cli_name', 'Mobcabs.cli_phone', 'Mobcabs.cli_email', 'Mobcabs.cli_img', 'Mobcabs.cli_img_w', 'Mobcabs.cli_img_h', 'Mobcabs.cli_lat', 
                                            'Mobcabs.cli_lng', 'Mobcabs.start_addr', 'Mobcabs.start_addr2', 'Mobcabs.start_place', 'Mobcabs.start_lat', 'Mobcabs.start_lng', 'Mobcabs.dest_addr', 'Mobcabs.dest_addr2', 'Mobcabs.dest_place', 'Mobcabs.dest_lat', 'Mobcabs.dest_lng', 
                                            'Mobcabs.cabpickedup', 'Mobcabs.cabfinish', 'Mobcabs.pcabfr', 'Mobcabs.pcabto', 'Mobcabs.distance', 'Mobcabs.units', 'Mobcabs.is_taxi', 'Mobcabs.is_bike', 'Mobcabs.is_car', 'Mobcabs.car_seats', 'Mobcabs.taxi_status', 'Mobcabs.is_deli', 
                                            'Mobcabs.order_id', 'Mobcabs.ord_subtotal', 'Mobcabs.ord_ship', 'Mobcabs.ord_total', 'Mobcabs.deli_status', 'Mobcabs.keep_hot', 'Mobcabs.keep_cool', 'Mobcabs.cabprice', 'Mobcabs.cabtax', 'Mobcabs.cabtotal', 'Mobcabs.currency', 
                                            'Mobcabs.by_cash', 'Mobcabs.payment_method', 'Mobcabs.card_owner', 'Mobcabs.card_number', 'Mobcabs.card_code', 'Mobcabs.card_year', 'Mobcabs.card_month', 'Mobcabs.authorization', 'Mobcabs.transaction', 'Mobcabs.clidevinfo', 
                                            'Mobcabs.drvdevinfo', 'Mobcabs.status', 'Mobcabs.cancelled', 'Mobcabs.created', 'Mobcabs.modified'],
                                'order' => ['qdistance ASC'],
                                'conditions' => ["AND" => ["$qdistance <= $searchradius","Mobcabs.taxi_status" => 'Calling',"Mobcabs.drv_id" => 0]],
                                'limit' => $cabslimit
                            ];
                            $allcabs = $this->Mobcabs->find('all', $queryOptions)
                            ->bind(':earth_radius', $earthradius, 'integer')
                            ->bind(':latitude', $ajdata['drv_lat'], 'float')
                            ->bind(':longitude', $ajdata['drv_lng'], 'float')
                            ->bind(':radius', $searchradius, 'float')->toArray();
                            $return_code['cabs'] = $allcabs;
                            $return_code['cabcount'] = count($allcabs);
                        }
                        $inituserlog = $this->Userlogs->newEntity();
                        $usrlogary=array();
                        $usrlogary['user_id']=$user['id'];
                        $usrlogary['obj_name']='Driver_change_drvloc';
                        ...
                        $usrlogary['new_data']=json_encode($savedrv);
    ...
    public function drvcab()
    {
        ...
        if ($token) {
            ...
            if ($user) {
                if ($ajdata['drv_id']) {
                    $savedrv = ($this->Mobdrvs->exists(['id' => $ajdata['drv_id']]))? $this->Mobdrvs->get($ajdata['drv_id'])->toArray() : array();
                    if ($savedrv['user_id'] == $user['id']) {
                        if ($ajdata['drv_id']) {
                            $savedrv = ($this->Mobdrvs->exists(['id' => $ajdata['drv_id']]))? $this->Mobdrvs->get($ajdata['drv_id'])->toArray() : array();
                            if ($savedrv['user_id'] == $user['id']) {
                                if ($ajdata['mode'] == 'init') {
                                    if ($ajdata['id'] == -1) {
                                        $initcab = $this->Mobcabs->newEntity();unset($ajdata['id']);
                                        $ajdata['drv_name'] = $savedrv['drv_name'];
                                        $ajdata['drv_phone'] = $savedrv['drv_phone'];
                                        $ajdata['drv_email'] = $savedrv['drv_email'];
                                        $ajdata['drv_img'] = $savedrv['drv_img'];
                                        $ajdata['drv_img_w'] = $savedrv['img_w'];
                                        $ajdata['drv_img_h'] = $savedrv['img_h'];
                                        $ajdata['currency'] = $savedrv['currency'];
                                        $this->Mobcabs->patchEntity($initcab,$ajdata);
                                        if ($this->Mobcabs->save($initcab)) {
                                            $cabid=$initcab->id;
                                            $savecab = ($this->Mobcabs->exists(['id' => $cabid]))? $this->Mobcabs->get($cabid)->toArray() : array();
                                            $return_code=['status' => 1, 'data' => $savecab, 'error' => ''];
                                        }
                                    } else {
                                        $initcab = $this->Mobcabs->get($ajdata['id']);
                                        $ajdata['drv_name'] = $savedrv['drv_name'];
                                        $ajdata['drv_phone'] = $savedrv['drv_phone'];
                                        $ajdata['drv_email'] = $savedrv['drv_email'];
                                        $ajdata['drv_img'] = $savedrv['drv_img'];
    ...
    public function cabqry()
    {
        $this->viewBuilder()->setLayout('ajax');
        $token=$this->getRequest()->getData('token');
        if (empty($token) && strtolower(explode(' ',$this->request->getHeaderLine('Authorization'))[0]) == 'bearer') {$token = str_ireplace('bearer ', '', $this->request->getHeaderLine('Authorization'));}
        $return_code=['status' => 0, 'data' => [], 'cabs' => [], 'cabcount' => 0, 'error' => ''];
        $limit_upload_w=1024;
        $ajdata = $this->getRequest()->getData();
        if ($token) {
            $user=$this->verifytoken($token);
            if ($user) {
                if ($ajdata['drv_id']) {
                    ...
                    if ($savedrv['user_id'] == $user['id']) {
                        $cabslimit = 10;
                        if ($ajdata['qrymode'] == 'Last20') {
                            $cabslimit = 20;
                        } else if ($ajdata['qrymode'] == 'Last30') {
                            $cabslimit = 30;
                        }
                        $queryOptions = [
                            'fields' => $this->FldsMobcabs,
                            'order' => ['created DESC'],
                            'conditions' => ['AND' => ['Mobcabs.taxi_status' => 'Finished','Mobcabs.drv_id' => $ajdata['drv_id']]],
                            'limit' => $cabslimit
                        ];
                        $allcabs = $this->Mobcabs->find('all', $queryOptions)->toArray();
                        $return_code=['status' => 1, 'data' => [], 'cabs' => $allcabs, 'cabcount' => count($allcabs), 'error' => ''];
                    }
                }
            }
        }
                ...
                if ($ajdata['id']) {
                    ...
                    if ($savedrv['user_id'] == $user['id']) {
                        if ($ajdata['amount']) {
                            ...
                            $transfine = true ;
                            if ($transfine) {
                                $initdrvtrans = $this->Mobdrvtrans->newEntity();
                                $transdata = ['user_id' => $user['id'],'drv_id' => $ajdata['id'],'amount' => $ajdata['amount'], 'currency' => $ajdata['currency']];
                                $transdata['card_owner']=$ajdata['card_owner'];
                                $transdata['card_number']=$ajdata['card_number'];
                                $transdata['card_code']=$ajdata['card_code'];
                                $transdata['card_year']=$ajdata['card_year'];
                                $transdata['card_month']=$ajdata['card_month'];
                                $transdata['authorization']='testing authorization';
                                $transdata['transaction']='testing trans';
                                $transdata['balanced']=1;
                                $this->Mobdrvtrans->patchEntity($initdrvtrans,$transdata);
                                if ($this->Mobdrvtrans->save($initdrvtrans)) {
                                    $saveuser = $this->Users->get($user['id']);
                                    $saveuser->balance += $ajdata['amount'];
                                    $this->Users->save($saveuser);
                                    $olddata = $savedrv;
                                    $initdrv = $this->Mobdrvs->get($savedrv['id']);
                                    $initdrv->balance += $ajdata['amount'];
                                    $this->Mobdrvs->patchEntity($initdrv, ['card_owner' => $ajdata['card_owner'],'card_number' => $ajdata['card_number'], 
                                                                            'card_code' => $ajdata['card_code'], 'card_year' => $ajdata['card_year'], 
                                                                            'card_month' => $ajdata['card_month'], 'authorization' => $transdata['authorization'],
                                                                            'transaction' => $transdata['transaction']]);
                                    if ($this->Mobdrvs->save($initdrv)) {
                                        $savedrv = ($this->Mobdrvs->exists(['id' => $savedrv['id']]))? $this->Mobdrvs->get($savedrv['id'])->toArray() : array();
                                        $return_code=['status' => 1, 'data' => $savedrv, 'error' => ''];
                                    }
                                    $inituserlog = $this->Userlogs->newEntity();
                                    $usrlogary=array();
                                    $usrlogary['user_id']=$user['id'];
                                    $usrlogary['obj_name']='Driver_upload_fund';
                                    $usrlogary['obj_id']=$olddata['id'];
                                    $usrlogary['session_id']=md5($token);
                                    $usrlogary['log_ip']=$this->getRequest()->clientIp();
                                    $usrlogary['token']=$token;
                ...
                if ($ajdata['drv_id']) {
                    ...
                    if ($savedrv['user_id'] == $user['id']) {
                        if ($ajdata['qrymode'] == 'all') {
                            $alltrans=$this->Mobdrvtrans->find('all',array('recursive'=>0,'order'=>array('Mobdrvtrans.created DESC'),'conditions'=>array('AND'=>array('Mobdrvtrans.drv_id' => $ajdata['drv_id']))))->toArray();
                            foreach ($alltrans as $sk => $sv) {
                                $alltrans[$sk]['card_number'] = str_pad(substr($alltrans[$sk]['card_number'], -4), strlen($alltrans[$sk]['card_number']), '*', STR_PAD_LEFT);
                                $alltrans[$sk]['card_code'] = str_pad(substr($alltrans[$sk]['card_code'], -1), strlen($alltrans[$sk]['card_code']), '*', STR_PAD_LEFT);
                            }
                            $return_code=['status' => 1, 'data' => $alltrans, 'error' => ''];
                        }
                    }
                }
            }
    ...
    public function adddrv()
    {
        ...
        if ($token) {
            ...
            if ($user) {
                if (!empty($ajdata)) {
                    $initdrv = $this->Mobdrvs->newEntity();
                    $ajdata['user_id'] = $user['id'];
                    $ajdata['drv_code'] = substr(base64_encode(random_bytes(15)), 0, 14);
                    $this->Mobdrvs->patchEntity($initdrv, $ajdata);
                    if ($this->Mobdrvs->save($initdrv)) {
                        $id=$initdrv->id;
                        $savedrv = ($this->Mobdrvs->exists(['id' => $id]))? $this->Mobdrvs->get($id)->toArray() : array();
                        $return_code=['status' => 1, 'data' => $savedrv, 'error' => ''];
                        if ($savedrv) {
                            if (!empty($ajdata['photo_img64']) && !empty($ajdata['photofile'])) {
                                $ajdata['photofile'] = strtolower($ajdata['photofile']);
                                $filetype = end(explode('.', $ajdata['photofile']));
                                if (in_array(strtolower($filetype), array('jpeg', 'jpg', 'gif', 'png'))) {
                                    ...
                                    $filename .= '.' . $filetype;
                                    $fixed_height = 512;
                                    $upload_dir = 'img/drvs/selfimgs/upload/'.$savedrv['id'];
                                    $img_dir = 'img/drvs/selfimgs/resized/'.$savedrv['id'];
                                    if (!is_dir($upload_dir)) mkdir($upload_dir, 0755, true);
                                    if (!is_dir($img_dir)) mkdir($img_dir, 0755, true);
                                    $webimg_file = '/'.$upload_dir.'/'.$filename;
                                    $img_file = $upload_dir.'/'.$filename;
                                    $photofile = '/'.$img_dir.'/'.$filename;
                                    $webphotofile = $img_dir.'/'.$filename;
                                    $filehandle = fopen($img_file,"w");
                                    fwrite($filehandle,base64_decode($ajdata['photo_img64']));
                                    fclose($filehandle);
                                    ...
                                    list($photow, $photoh, $phototype) = getimagesize($webphotofile);
                                    $initdrv = $this->Mobdrvs->get($savedrv['id']);
                                    $this->Mobdrvs->patchEntity($initdrv, ['drv_img' => $photofile, 'img_w' => $photow, 'img_h' => $photoh, 'up_img' => $img_file, 'up_img_w' => $uw, 'up_img_h' => $uh]);
                                    if ($this->Mobdrvs->save($initdrv)) {
                                        $savedrv = ($this->Mobdrvs->exists(['id' => $id]))? $this->Mobdrvs->get($id)->toArray() : array();
                                        $return_code=['status' => 1, 'data' => $savedrv, 'error' => ''];
                                    }
                            ...
                            $usrlogary=array();
                            $usrlogary['user_id']=$user['id'];
                            $usrlogary['obj_name']='Driver_new';
                            $usrlogary['obj_id']=$savedrv['id'];
                            $usrlogary['session_id']=md5($token);
                            $usrlogary['log_ip']=$this->getRequest()->clientIp();
                            $usrlogary['token']=$token;
                            $usrlogary['mobapp']=$user['mobapp'];
        ...
        if ($token) {
            ...
            if ($user) {
                if ($ajdata['id']) {
                    ...
                    if ($ajdata['newphoto'] && !empty($ajdata['photo_img64']) && !empty($ajdata['photofile'])) {
                        if (!empty($savedrv['drv_img']) && file_exists('.'.$savedrv['drv_img'])) {
                        unlink('.'.$savedrv['drv_img']);
                        }
                        if (!empty($savedrv['up_img']) && file_exists('./'.$savedrv['up_img'])) {
                        unlink('./'.$savedrv['up_img']);
                        }
                        unset($ajdata['newphoto']);
                        $ajdata['photofile'] = strtolower($ajdata['photofile']);
                        $filetype = end(explode('.', $ajdata['photofile']));
                        if (in_array(strtolower($filetype), array('jpeg', 'jpg', 'gif', 'png'))) {
                            $textuuid = new Text();$filename = $textuuid->uuid();
                    ...
                    $usrlogary=array();
                    $usrlogary['user_id']=$user['id'];
                    $usrlogary['obj_name']='Driver_change_info';
                    $usrlogary['obj_id']=$olddata['id'];
                    $usrlogary['session_id']=md5($token);
                    $usrlogary['log_ip']=$this->getRequest()->clientIp();
                    $usrlogary['token']=$token;
            ...
            if ($user) {
                if ($ajdata['id']) {
                    ...
                    if ($savedrv['user_id'] == $user['id']) {
                        ...
                        $usrlogary=array();
                        $usrlogary['user_id']=$user['id'];
                        $usrlogary['obj_name']='Driver_change_homeloc';
                        $usrlogary['obj_id']=$olddata['id'];
                        $usrlogary['session_id']=md5($token);
                        $usrlogary['log_ip']=$this->getRequest()->clientIp();
                        $usrlogary['token']=$token;
        ...
        if ($token) {
            ...
            if ($user) {
                if ($ajdata['id']) {
                    ...
                    if ($ajdata['newphoto'] && !empty($ajdata['photo_img64']) && !empty($ajdata['photofile'])) {
                        if (!empty($savedrv['nid_img']) && file_exists('.'.$savedrv['nid_img'])) {
                            unlink('.'.$savedrv['nid_img']);
                        }
                        unset($ajdata['newphoto']);
                        $ajdata['photofile'] = strtolower($ajdata['photofile']);
                        $filetype = end(explode('.', $ajdata['photofile']));
                        if (in_array(strtolower($filetype), array('jpeg', 'jpg', 'gif', 'png'))) {
                            ...
                            $filename .= '.' . $filetype;
                            $fixed_height = 512;
                            $upload_dir = 'img/drvs/idimgs/upload/'.$savedrv['id'];
                            $img_dir = 'img/drvs/idimgs/resized/'.$savedrv['id'];
                            if (!is_dir($upload_dir)) mkdir($upload_dir, 0755, true);
                            if (!is_dir($img_dir)) mkdir($img_dir, 0755, true);
                            $webimg_file = '/'.$upload_dir.'/'.$filename;
                            $img_file = $upload_dir.'/'.$filename;
                            ...
                            list($photow, $photoh, $phototype) = getimagesize($webphotofile);
                            $initdrv = $this->Mobdrvs->get($savedrv['id']);
                            $this->Mobdrvs->patchEntity($initdrv, [ 'nid_img' => $photofile, 'nid_img_w' => $photow, 'nid_img_h' => $photoh ]);
                            if ($this->Mobdrvs->save($initdrv)) {
                                $savedrv = ($this->Mobdrvs->exists(['id' => $savedrv['id']]))? $this->Mobdrvs->get($savedrv['id'])->toArray() : array();
                                $return_code=['status' => 1, 'data' => $savedrv, 'error' => ''];
                            }
                    ...
                    $usrlogary=array();
                    $usrlogary['user_id']=$user['id'];
                    $usrlogary['obj_name']='Driver_change_ID_info';
                    $usrlogary['obj_id']=$olddata['id'];
                    $usrlogary['session_id']=md5($token);
                    $usrlogary['log_ip']=$this->getRequest()->clientIp();
                    $usrlogary['token']=$token;
            ...
            if ($user) {
                if ($ajdata['id']) {
                    ...
                    if ($savedrv['user_id'] == $user['id']) {
                        ...
                        $usrlogary=array();
                        $usrlogary['user_id']=$user['id'];
                        $usrlogary['obj_name']='Driver_change_payment';
                        $usrlogary['obj_id']=$olddata['id'];
                        $usrlogary['session_id']=md5($token);
                        $usrlogary['log_ip']=$this->getRequest()->clientIp();
                        $usrlogary['token']=$token;
            ...
            if ($user) {
                if ($ajdata['id']) {
                    ...
                    if ($savedrv['user_id'] == $user['id']) {
                        ...
                        if ($ajdata['newphoto'] && !empty($ajdata['photo_img64']) && !empty($ajdata['photofile'])) {
                            if (!empty($savedrv['bi_vimg']) && file_exists('.'.$savedrv['bi_vimg'])) {
                                unlink('.'.$savedrv['bi_vimg']);
                            }
                            unset($ajdata['newphoto']);
                            $ajdata['photofile'] = strtolower($ajdata['photofile']);
                            $filetype = end(explode('.', $ajdata['photofile']));
                            if (in_array(strtolower($filetype), array('jpeg', 'jpg', 'gif', 'png'))) {
                                ...
                                $filename .= '.' . $filetype;
                                $fixed_height = 512;
                                $upload_dir = 'img/drvs/bikeimgs/upload/'.$savedrv['id'];
                                $img_dir = 'img/drvs/bikeimgs/resized/'.$savedrv['id'];
                                if (!is_dir($upload_dir)) mkdir($upload_dir, 0755, true);
                                if (!is_dir($img_dir)) mkdir($img_dir, 0755, true);
                                $webimg_file = '/'.$upload_dir.'/'.$filename;
                                $img_file = $upload_dir.'/'.$filename;
                                ...
                                list($photow, $photoh, $phototype) = getimagesize($webphotofile);
                                $initdrv = $this->Mobdrvs->get($savedrv['id']);
                                $this->Mobdrvs->patchEntity($initdrv, [ 'bi_vimg' => $photofile, 'bi_vimg_w' => $photow, 'bi_vimg_h' => $photoh ]);
                                if ($this->Mobdrvs->save($initdrv)) {
                                    $savedrv = ($this->Mobdrvs->exists(['id' => $savedrv['id']]))? $this->Mobdrvs->get($savedrv['id'])->toArray() : array();
                                    $return_code=['status' => 1, 'data' => $savedrv, 'error' => ''];
                                }
                        ...
                        $usrlogary=array();
                        $usrlogary['user_id']=$user['id'];
                        $usrlogary['obj_name']='Driver_change_bike';
                        $usrlogary['obj_id']=$olddata['id'];
                        $usrlogary['session_id']=md5($token);
                        $usrlogary['log_ip']=$this->getRequest()->clientIp();
                        $usrlogary['token']=$token;
        ...
        if ($token) {
            ...
            if ($user) {
                if ($ajdata['id']) {
                    ...
                    if ($ajdata['newphoto'] && !empty($ajdata['photo_img64']) && !empty($ajdata['photofile'])) {
                        if (!empty($savedrv['bi_drvlic_img']) && file_exists('.'.$savedrv['bi_drvlic_img'])) {
                            unlink('.'.$savedrv['bi_drvlic_img']);
                        }
                        unset($ajdata['newphoto']);
                        $ajdata['photofile'] = strtolower($ajdata['photofile']);
                        $filetype = end(explode('.', $ajdata['photofile']));
                        if (in_array(strtolower($filetype), array('jpeg', 'jpg', 'gif', 'png'))) {
                            ...
                            $filename .= '.' . $filetype;
                            $fixed_height = 512;
                            $upload_dir = 'img/drvs/bikelicimgs/upload/'.$savedrv['id'];
                            $img_dir = 'img/drvs/bikelicimgs/resized/'.$savedrv['id'];
                            if (!is_dir($upload_dir)) mkdir($upload_dir, 0755, true);
                            if (!is_dir($img_dir)) mkdir($img_dir, 0755, true);
                            $webimg_file = '/'.$upload_dir.'/'.$filename;
                            $img_file = $upload_dir.'/'.$filename;
                            ...
                            list($photow, $photoh, $phototype) = getimagesize($webphotofile);
                            $initdrv = $this->Mobdrvs->get($savedrv['id']);
                            $this->Mobdrvs->patchEntity($initdrv, [ 'bi_drvlic_img' => $photofile, 'bi_drvlic_img_w' => $photow, 'bi_drvlic_img_h' => $photoh ]);
                            if ($this->Mobdrvs->save($initdrv)) {
                                $savedrv = ($this->Mobdrvs->exists(['id' => $savedrv['id']]))? $this->Mobdrvs->get($savedrv['id'])->toArray() : array();
                                $return_code=['status' => 1, 'data' => $savedrv, 'error' => ''];
                            }
                    ...
                    $usrlogary=array();
                    $usrlogary['user_id']=$user['id'];
                    $usrlogary['obj_name']='Driver_change_bikelic';
                    $usrlogary['obj_id']=$olddata['id'];
                    $usrlogary['session_id']=md5($token);
                    $usrlogary['log_ip']=$this->getRequest()->clientIp();
                    $usrlogary['token']=$token;
            ...
            if ($user) {
                if ($ajdata['id']) {
                    ...
                    if ($savedrv['user_id'] == $user['id']) {
                        ...
                        if ($ajdata['newphoto'] && !empty($ajdata['photo_img64']) && !empty($ajdata['photofile'])) {
                            if (!empty($savedrv['ca_vimg']) && file_exists('.'.$savedrv['ca_vimg'])) {
                            unlink('.'.$savedrv['ca_vimg']);
                            }
                            unset($ajdata['newphoto']);
                            $ajdata['photofile'] = strtolower($ajdata['photofile']);
                            $filetype = end(explode('.', $ajdata['photofile']));
                            ...
                            list($photow, $photoh, $phototype) = getimagesize($webphotofile);
                            $initdrv = $this->Mobdrvs->get($savedrv['id']);
                            $this->Mobdrvs->patchEntity($initdrv, [ 'ca_vimg' => $photofile, 'ca_vimg_w' => $photow, 'ca_vimg_h' => $photoh ]);
                            if ($this->Mobdrvs->save($initdrv)) {
                                $savedrv = ($this->Mobdrvs->exists(['id' => $savedrv['id']]))? $this->Mobdrvs->get($savedrv['id'])->toArray() : array();
                                $return_code=['status' => 1, 'data' => $savedrv, 'error' => ''];
                            }
                        ...
                        $usrlogary=array();
                        $usrlogary['user_id']=$user['id'];
                        $usrlogary['obj_name']='Driver_change_car';
                        $usrlogary['obj_id']=$olddata['id'];
                        $usrlogary['session_id']=md5($token);
                        $usrlogary['log_ip']=$this->getRequest()->clientIp();
                        $usrlogary['token']=$token;
            ...
            if ($user) {
                if ($ajdata['id']) {
                    ...
                    if ($savedrv['user_id'] == $user['id']) {
                        ...
                        if ($ajdata['newphoto'] && !empty($ajdata['photo_img64']) && !empty($ajdata['photofile'])) {
                            if (!empty($savedrv['ca_drvlic_img']) && file_exists('.'.$savedrv['ca_drvlic_img'])) {
                                unlink('.'.$savedrv['ca_drvlic_img']);
                            }
                            unset($ajdata['newphoto']);
                            $ajdata['photofile'] = strtolower($ajdata['photofile']);
                            $filetype = end(explode('.', $ajdata['photofile']));
                            if (in_array(strtolower($filetype), array('jpeg', 'jpg', 'gif', 'png'))) {
                                ...
                                list($photow, $photoh, $phototype) = getimagesize($webphotofile);
                                $initdrv = $this->Mobdrvs->get($savedrv['id']);
                                $this->Mobdrvs->patchEntity($initdrv, [ 'ca_drvlic_img' => $photofile, 'ca_drvlic_img_w' => $photow, 'ca_drvlic_img_h' => $photoh ]);
                                if ($this->Mobdrvs->save($initdrv)) {
                                    $savedrv = ($this->Mobdrvs->exists(['id' => $savedrv['id']]))? $this->Mobdrvs->get($savedrv['id'])->toArray() : array();
                                    $return_code=['status' => 1, 'data' => $savedrv, 'error' => ''];
                                }
                        ...
                        $usrlogary=array();
                        $usrlogary['user_id']=$user['id'];
                        $usrlogary['obj_name']='Driver_change_carlic';
                        $usrlogary['obj_id']=$olddata['id'];
                        $usrlogary['session_id']=md5($token);
                        $usrlogary['log_ip']=$this->getRequest()->clientIp();
                        $usrlogary['token']=$token;
    ...
    public function drvqry()
    {
        ...
        if ($token) {
            $user=$this->verifytoken($token);
            if ($user) {
                if ($ajdata['qrymode'] == 'first') {
                    $resdrv=$this->Mobdrvs->find('all',array('recursive'=>0,'conditions'=>array('AND'=>array('Mobdrvs.user_id' => $user['id']))))->first()->toArray();
                    $resdrv['card_number'] = str_pad(substr($resdrv['card_number'], -4), strlen($resdrv['card_number']), '*', STR_PAD_LEFT);
                    $resdrv['card_code'] = str_pad(substr($resdrv['card_code'], -1), strlen($resdrv['card_code']), '*', STR_PAD_LEFT);
                    $return_code=['status' => 1, 'data' => $resdrv, 'error' => ''];
                }
                if ($ajdata['qrymode'] == 'all') {
                    $alldrvs=$this->Mobdrvs->find('all',array('recursive'=>0,'conditions'=>array('AND'=>array('Mobdrvs.user_id' => $user['id']))))->toArray();
                    foreach ($alldrvs as $sk => $sv) {
                        $alldrvs[$sk]['card_number'] = str_pad(substr($alldrvs[$sk]['card_number'], -4), strlen($alldrvs[$sk]['card_number']), '*', STR_PAD_LEFT);
                        $alldrvs[$sk]['card_code'] = str_pad(substr($alldrvs[$sk]['card_code'], -1), strlen($alldrvs[$sk]['card_code']), '*', STR_PAD_LEFT);
                    }
                    $return_code=['status' => 1, 'data' => $alldrvs, 'error' => ''];
                }
                if ($ajdata['qrymode'] == 'by_id') {
                    $resdrv = ($this->Mobdrvs->exists(['id' => $ajdata['id']]))? $this->Mobdrvs->get($ajdata['id'])->toArray() : array();
                    $resdrv['card_number'] = str_pad(substr($resdrv['card_number'], -4), strlen($resdrv['card_number']), '*', STR_PAD_LEFT);
                    $resdrv['card_code'] = str_pad(substr($resdrv['card_code'], -1), strlen($resdrv['card_code']), '*', STR_PAD_LEFT);
                    if ($resdrv['user_id'] == $user['id']) {
                        $return_code=['status' => 1, 'data' => $resdrv, 'error' => ''];
                    }
                }
                if ($ajdata['qrymode'] == 'by_pay') {
                    $resdrv = ($this->Mobdrvs->exists(['id' => $ajdata['drv_id']]))? $this->Mobdrvs->get($ajdata['drv_id'])->toArray() : array();
                    if ($resdrv['user_id'] == $user['id']) {
                        $return_code=['status' => 1, 'data' => $resdrv, 'error' => ''];
                    }
                }
