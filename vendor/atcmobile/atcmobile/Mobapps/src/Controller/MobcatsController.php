 ...
 */
class MobcatsController extends AppController
{
    ...
    public function beforeFilter(Event $event)
    {
        ...
        $this->Auth->allow(['addcat', 'delcat', 'editcat', 'getcat', 'catqry']);
    }
                ...
                if ($ajdata['id']) {
                    ...
                    if ($savestore['user_id'] == $user['id']) {
                        ...
                        if ($this->Mobcats->save($initcat)) {
                            ...
                            $allprods=$this->Mobprods->find('all',array('recursive'=>0,'order'=>array('Mobprods.sort ASC'),
                            'fields'=>$this->FldsMobprods,
                            'conditions'=>array('AND'=>array('Mobprods.cat_id' => $category['id']))));
                            if ($allprods->count() > 0) {
                                $category['prods']=$allprods;
                                // $category['prods']=[];
                            } else {
                                $category['prods']=[];
                            }
            ...
            if ($user) {
                if ($ajdata['qrymode'] == 'all') {
                    if ($ajdata['filter'] != '') {
                        ...
                        $allcat=$this->Mobcats->find('all',array('recursive'=>0,'order'=>array('Mobcats.sort ASC'),
                            'fields'=>$this->FldsMobcats,
                            'conditions'=>array('AND'=>array('Mobcats.store_id' => $ajdata['store_id']))))->toArray();
                    }
                ...
                if ($ajdata['qrymode'] == 'by_id') {
                    ...
                    if ($this->Mobcats->exists(['id' => $ajdata['id']])) {
                        $rescat['prods'] = [];
                    }
