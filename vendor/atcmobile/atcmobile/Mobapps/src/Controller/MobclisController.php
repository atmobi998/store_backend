 ...
 */
class MobclisController extends AppController
{
    ...
    public function beforeFilter(Event $event)
    {
        ...
        $this->Auth->allow(['login','token','clicab','cabqry','drvqry','upfund','transqry','balance','addcli','delcli','editcli','getcli','cliqry','homeloc','cliloc','editnid','editpay']);
    }

    public function cliloc()
    {
        ...
        if ($token) {
            ...
            if ($user) {
                if ($ajdata['id']) {
                    $savecli = ($this->Mobclis->exists(['id' => $ajdata['id']]))? $this->Mobclis->get($ajdata['id'])->toArray() : array();
                    if ($savecli['user_id'] == $user['id']) {
                        $olddata = $savecli;
                        ...
                        if ($ajdata['newcode'] == '1') {
                            $ajdata['cli_code'] = substr(base64_encode(random_bytes(15)), 0, 14);
                        }
                        unset($ajdata['photo_img64']);unset($ajdata['photofile']);
                        $initcli = $this->Mobclis->get($savecli['id']);
                        if (empty($savecli['cli_code'])) {
                            $ajdata['cli_code'] = substr(base64_encode(random_bytes(15)), 0, 14);
                        }
                        $this->Mobclis->patchEntity($initcli, $ajdata);
                        if ($this->Mobclis->save($initcli)) {
                            $savecli = ($this->Mobclis->exists(['id' => $savecli['id']]))? $this->Mobclis->get($savecli['id'])->toArray() : array();
                            $return_code=['status' => 1, 'data' => $savecli, 'error' => ''];
                        }
                        if ($ajdata['cab_id'] > 0) {
                            $cabid=$ajdata['cab_id'];
                            $initcab = $this->Mobcabs->get($cabid);$cabdata=[];
                            $cabdata['cli_name'] = $savecli['cli_name'];
                            $cabdata['cli_phone'] = $savecli['cli_phone'];
                            $cabdata['cli_email'] = $savecli['cli_email'];
                            $cabdata['cli_img'] = $savecli['cli_img'];
                            $cabdata['cli_img_w'] = $savecli['img_w'];
                            $cabdata['cli_img_h'] = $savecli['img_h'];
                            $cabdata['currency'] = $savecli['currency'];
                            $cabdata['cli_lat'] = $ajdata['cli_lat'];
                            $cabdata['cli_lng'] = $ajdata['cli_lng'];
                        ...
                        $usrlogary['new_data']=json_encode($savecli);
    ...
    public function clicab()
    {
        ...
        if ($token) {
            ...
            if ($user) {
                if ($ajdata['cli_id']) {
                    $savecli = ($this->Mobclis->exists(['id' => $ajdata['cli_id']]))? $this->Mobclis->get($ajdata['cli_id'])->toArray() : array();
                    if ($savecli['user_id'] == $user['id']) {
                        if ($ajdata['mode'] == 'init') {
                            if ($ajdata['id'] == -1) {
                                $initcab = $this->Mobcabs->newEntity();unset($ajdata['id']);
                                $ajdata['cli_name'] = $savecli['cli_name'];
                                $ajdata['cli_phone'] = $savecli['cli_phone'];
                                $ajdata['cli_email'] = $savecli['cli_email'];
                                $ajdata['cli_img'] = $savecli['cli_img'];
                                $ajdata['cli_img_w'] = $savecli['img_w'];
                                $ajdata['cli_img_h'] = $savecli['img_h'];
                                $ajdata['currency'] = $savecli['currency'];
                ...
                if ($ajdata['id']) {
                    ...
                    if ($savecli['user_id'] == $user['id']) {
                        if ($ajdata['amount']) {
                            ...
                            $transfine = true ;
                            if ($transfine) {
                                $initclitrans = $this->Mobclitrans->newEntity();
                                $transdata = ['user_id' => $user['id'],'cli_id' => $ajdata['id'],'amount' => $ajdata['amount'], 'currency' => $ajdata['currency']];
                                ...
                                $this->Mobclitrans->patchEntity($initclitrans,$transdata);
                                if ($this->Mobclitrans->save($initclitrans)) {
                                    $saveuser = $this->Users->get($user['id']);
                                    $saveuser->balance += $ajdata['amount'];
                                    $this->Users->save($saveuser);
                                    $olddata = $savecli;
                                    $initcli = $this->Mobclis->get($savecli['id']);
                                    $initcli->balance += $ajdata['amount'];
                                    $this->Mobclis->patchEntity($initcli, ['card_owner' => $ajdata['card_owner'],'card_number' => $ajdata['card_number'], 
                ...
                if ($ajdata['cli_id']) {
                    ...
                    if ($savecli['user_id'] == $user['id']) {
                        if ($ajdata['qrymode'] == 'all') {
                            $alltrans=$this->Mobclitrans->find('all',array('recursive'=>0,'order'=>array('Mobclitrans.created DESC'),'conditions'=>array('AND'=>array('Mobclitrans.cli_id' => $ajdata['cli_id']))))->toArray();
    ...
    public function addcli()
    {
        ...
        if ($token) {
            ...
            if ($user) {
                if (!empty($ajdata)) {
                    $initcli = $this->Mobclis->newEntity();
                    $ajdata['user_id'] = $user['id'];
                    $ajdata['cli_code'] = substr(base64_encode(random_bytes(15)), 0, 14);
                    $this->Mobclis->patchEntity($initcli, $ajdata);
                    if ($this->Mobclis->save($initcli)) {
                        $id=$initcli->id;
                        $savecli = ($this->Mobclis->exists(['id' => $id]))? $this->Mobclis->get($id)->toArray() : array();
                        $return_code=['status' => 1, 'data' => $savecli, 'error' => ''];
                        if ($savecli) {
                            if (!empty($ajdata['photo_img64']) && !empty($ajdata['photofile'])) {
                                ...
                                if (in_array(strtolower($filetype), array('jpeg', 'jpg', 'gif', 'png'))) {
                                    ...
                                    $filename .= '.' . $filetype;
                                    $fixed_height = 512;
                                    $upload_dir = 'img/clis/selfimgs/upload/'.$savecli['id'];
                                    $img_dir = 'img/clis/selfimgs/resized/'.$savecli['id'];
                                    ...
                                    $initcli = $this->Mobclis->get($savecli['id']);
                                    $this->Mobclis->patchEntity($initcli, ['cli_img' => $photofile, 'img_w' => $photow, 'img_h' => $photoh, 'up_img' => $img_file, 'up_img_w' => $uw, 'up_img_h' => $uh]);
                                    if ($this->Mobclis->save($initcli)) {
                                        $savecli = ($this->Mobclis->exists(['id' => $id]))? $this->Mobclis->get($id)->toArray() : array();
                                        $return_code=['status' => 1, 'data' => $savecli, 'error' => ''];
                                    }
                ...
                if ($ajdata['id']) {
                    ...
                    if ($savecli['user_id'] == $user['id']) {
                        ...
                        if ($ajdata['newphoto'] && !empty($ajdata['photo_img64']) && !empty($ajdata['photofile'])) {
                            if (!empty($savecli['cli_img']) && file_exists('.'.$savecli['cli_img'])) {
                                unlink('.'.$savecli['cli_img']);
                            }
                            if (!empty($savecli['up_img']) && file_exists('./'.$savecli['up_img'])) {
                                unlink('./'.$savecli['up_img']);
                            }
            ...
            if ($user) {
                if ($ajdata['id']) {
                    ...
                    if ($savecli['user_id'] == $user['id']) {
                        ...
                        if ($ajdata['newphoto'] && !empty($ajdata['photo_img64']) && !empty($ajdata['photofile'])) {
                            if (!empty($savecli['nid_img']) && file_exists('.'.$savecli['nid_img'])) {
                                unlink('.'.$savecli['nid_img']);
                            }
                            unset($ajdata['newphoto']);
                            $ajdata['photofile'] = strtolower($ajdata['photofile']);
                            $filetype = end(explode('.', $ajdata['photofile']));
                            if (in_array(strtolower($filetype), array('jpeg', 'jpg', 'gif', 'png'))) {
                                ...
                                $filename .= '.' . $filetype;
                                $fixed_height = 512;
                                $upload_dir = 'img/clis/idimgs/upload/'.$savecli['id'];
                                $img_dir = 'img/clis/idimgs/resized/'.$savecli['id'];
                                if (!is_dir($upload_dir)) mkdir($upload_dir, 0755, true);
                                if (!is_dir($img_dir)) mkdir($img_dir, 0755, true);
                                $webimg_file = '/'.$upload_dir.'/'.$filename;
                                $img_file = $upload_dir.'/'.$filename;
                                ...
                                list($photow, $photoh, $phototype) = getimagesize($webphotofile);
                                $initcli = $this->Mobclis->get($savecli['id']);
                                $this->Mobclis->patchEntity($initcli, [ 'nid_img' => $photofile, 'nid_img_w' => $photow, 'nid_img_h' => $photoh ]);
                                if ($this->Mobclis->save($initcli)) {
                                    $savecli = ($this->Mobclis->exists(['id' => $savecli['id']]))? $this->Mobclis->get($savecli['id'])->toArray() : array();
                                    $return_code=['status' => 1, 'data' => $savecli, 'error' => ''];
                                }
        ...
        if ($token) {
            ...
            if ($user) {
                if ($ajdata['qrymode'] == 'first') {
                    $rescli=$this->Mobclis->find('all',array('recursive'=>0,'conditions'=>array('AND'=>array('Mobclis.user_id' => $user['id']))))->first()->toArray();
                    $rescli['card_number'] = str_pad(substr($rescli['card_number'], -4), strlen($rescli['card_number']), '*', STR_PAD_LEFT);
                    $rescli['card_code'] = str_pad(substr($rescli['card_code'], -1), strlen($rescli['card_code']), '*', STR_PAD_LEFT);
                    $return_code=['status' => 1, 'data' => $rescli, 'error' => ''];
                }
                if ($ajdata['qrymode'] == 'all') {
                    $alldrvs=$this->Mobclis->find('all',array('recursive'=>0,'conditions'=>array('AND'=>array('Mobclis.user_id' => $user['id']))))->toArray();
                ...
                if ($ajdata['qrymode'] == 'by_id') {
                    $rescli = ($this->Mobclis->exists(['id' => $ajdata['id']]))? $this->Mobclis->get($ajdata['id'])->toArray() : array();
                    $rescli['card_number'] = str_pad(substr($rescli['card_number'], -4), strlen($rescli['card_number']), '*', STR_PAD_LEFT);
                    $rescli['card_code'] = str_pad(substr($rescli['card_code'], -1), strlen($rescli['card_code']), '*', STR_PAD_LEFT);
                    if ($rescli['user_id'] == $user['id']) {
                        $return_code=['status' => 1, 'data' => $rescli, 'error' => ''];
                    }
                }
                if ($ajdata['qrymode'] == 'by_pay') {
                    $rescli = ($this->Mobclis->exists(['id' => $ajdata['cli_id']]))? $this->Mobclis->get($ajdata['cli_id'])->toArray() : array();
                    if ($rescli['user_id'] == $user['id']) {
                        $return_code=['status' => 1, 'data' => $rescli, 'error' => ''];
                    }
                }
