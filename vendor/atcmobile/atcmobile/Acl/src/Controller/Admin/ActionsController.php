...
namespace Atcmobapp\Acl\Controller\Admin;
...
class ActionsController extends AppController
{
    ...
    public function initialize()
    {
        ...
        $this->Acos = TableRegistry::get('Atcmobapp/Acl.Acos');
    }
    ...
    public function add()
    {
        ...
        if ($this->getRequest()->is('post')) {
            ...
            if ($this->Acos->save($aco)) {
                $this->Flash->success(sprintf(__d('atcmobile', 'The %s has been saved'), $acoType));

                return $this->redirect(['action' => 'index']);
            } else {
                $this->Flash->error(sprintf(__d('atcmobile', 'The %s could not be saved. Please, try again.'), $acoType));
            }
        }
    ...
    public function delete($id = null)
    {
        ...
        if ($this->Acos->delete($aco)) {
            $this->Flash->success(__d('atcmobile', 'Action deleted'));
    ...
    public function move($id, $direction = 'up', $step = '1')
    {
        ...
        if ($direction == 'up') {
            if ($this->Acos->moveUp($aco)) {
                $this->Flash->success(__d('atcmobile', 'Action moved up'));

                return $this->redirect(['action' => 'index']);
            }
        } else {
            if ($this->Acos->moveDown($aco)) {
                $this->Flash->success(__d('atcmobile', 'Action moved down'));
    ...
    public function generate()
    {
        ...
        if ($this->getRequest()->getQuery('permissions')) {
            return $this->redirect(['plugin' => 'Atcmobapp/Acl', 'controller' => 'Permissions', 'action' => 'index']);
        } else {
            return $this->redirect(['action' => 'index']);
        }
    }
