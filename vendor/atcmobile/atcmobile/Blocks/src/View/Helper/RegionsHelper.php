...
namespace Atcmobapp\Blocks\View\Helper;
 ...
 *
 * @category Helper
 * @package  Atcmobapp.Blocks.View.Helper
 * @version  1.0
...
class RegionsHelper extends Helper
{
    ...
    public function block(Block $block, $regionAlias = null, $options = [])
    {
        ...
        $defaultElement = 'Atcmobapp/Blocks.block';

        $element = $block->element;
        $exists = $this->_View->elementExists($element);

        $event = Atcmobapp::dispatchEvent('Helper.Regions.beforeSetBlock', $this->_View, [
            'content' => $block->body,
        ]);
        ...
        Atcmobapp::dispatchEvent('Helper.Regions.afterSetBlock', $this->_View, [
            'content' => &$blockOutput,
        ]);
