...
namespace Atcmobapp\Dashboards\Controller\Admin;
 ...
 *
 * @category Controller
 * @package  Atcmobapp.Dashboards.Controller
 * @version  2.2
...
class DashboardsController extends AppController
{
    ...
    public function beforeRender(Event $event)
    {
        ...
        $this->viewBuilder()->setHelpers([
            'Atcmobapp/Dashboards.Dashboards',
        ]);
    }
    ...
    {
        if (!$id) {
            $this->Flash->error(__d('atcmobile', 'Invalid id for Dashboard'));

            return $this->redirect(['action' => 'index']);
        }
        $entity = $this->Dashboards->get($id);
        if ($this->Dashboards->delete($entity)) {
            $this->Flash->success(__d('atcmobile', 'Dashboard deleted'));
    ...
    {
        $this->Atcmobapp->fieldToggle($this->Dashboards, $id, $status);
    }
